...
typedef struct
{
	...
	char          col3[20];
	int           count;
} PCHEADER_RANKING_LIST, * PCPHEADER_RANKING_LIST;
...
typedef struct
{
	...
	char value3[25];
	bool CheckOnline;
} PCCREATE_RANKING_INFO, * PCPCREATE_RANKING_INFO;
...
typedef struct
{
	PSWMSG_HEAD Header;
	char NameChar[20];
	BYTE Inventory[MAX_NEW_EQUIPMENT][12];
	BYTE Class;
	int TotalPoint;
	int Level;
	int MasterLevel;
...
} DATA_VIEW_CHARINFO, * LPDATA_VIEW_CHARINFO;
#pragma pack(pop)
...
#define SendRequestCustomJewelBank(Type, Count) \
{ \
