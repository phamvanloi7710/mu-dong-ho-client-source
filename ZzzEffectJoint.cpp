...
void MoveJoint(JOINT* o, int iIndex)
{
	...
	case BITMAP_JOINT_ENERGY:
		...
		case 46:
		{
			if (o->LifeTime >= 100 && o->SubType != 44)
			{
				if (o->SubType == 0)
				{
					...
					o->Position[2] += (AdvanceByDelta(6.f));
				}
				...
				else if (o->SubType == 12)
				{
					...
					o->Position[0] += AdvanceByDelta((int)o->MultiUse * (rand() % 5));
					o->Position[1] += AdvanceByDelta((int)o->MultiUse * (rand() % 5));
					o->Position[2] += (AdvanceByDelta(6.f));
				}
				else if (o->SubType == 16 || o->SubType == 46)
				{
					o->Angle[2] -= AdvanceByDelta(o->MultiUse * 20.f);
					o->Position[0] += AdvanceByDelta(o->MultiUse * (rand() % 5 + 3));
					o->Position[1] += AdvanceByDelta(o->MultiUse * (rand() % 5 + 3));
					o->Position[2] += (AdvanceByDelta(8.f));
				}
				else if (o->SubType == 13)
				{
				...
				{
					o->Angle[2] -= (AdvanceByDelta(20.f));
					o->Position[0] += AdvanceByDelta((int)o->MultiUse * rand() % 5 + cosf(Q_PI / (float)(rand() % 180) * 20.0f));
					o->Position[1] += AdvanceByDelta((int)o->MultiUse * rand() % 5 + sinf(Q_PI / (float)(rand() % 180) * 20.0f));
					o->Position[2] += AdvanceByDelta(6.f);
				}
				else
				{
					o->Angle[2] -= AdvanceByDelta(10.f);
					o->Position[2] += AdvanceByDelta(5.f);
				}
			}
			else if (o->SubType == 44 && o->LifeTime <= 120 && o->LifeTime >= 10)
			{
				...
				{
					o->Velocity += AdvanceByDelta(12.f);
					if (o->Velocity >= 20.f)
						o->Velocity = 20.f;

					o->Position[0] += AdvanceByDelta(cosf(Q_PI / (float)(rand() % 180 + 150)));
					o->Position[1] += AdvanceByDelta(sinf(Q_PI / (float)(rand() % 180 + 150)));
			...
			else
			{
				...
				{
					o->Velocity += AdvanceByDelta(7.f);
					if (o->Velocity >= 40.f)
						o->Velocity = 40.f;
				}
				else
				{
					o->Velocity += AdvanceByDelta(5.f);
					if (o->Velocity >= 30.f)
						o->Velocity = 30.f;
				}
				...
				{
					o->Position[2] += AdvanceByDelta(o->Velocity);
				}
				else if (o->SubType == 44)
				{
					VectorCopy(o->Target->Position, o->TargetPosition);
		...
		case 42:
		{
			if (o->LifeTime >= 60)
			{
				...
				o->Light[0] += AdvanceByDelta(0.002f);
				o->Light[1] += AdvanceByDelta(0.0015f);
				o->Light[2] += AdvanceByDelta(0.011f);
			}
			else
			{
	...
	case BITMAP_JOINT_HEALING:
		...
		else if (o->SubType == 9)
		{
			...
			o->MultiUse += AdvanceByDelta(2.f);
			o->Direction[0] -= AdvanceByDelta(1.f);
			...
			{
				VectorScale(o->Light, ReduceWithDelta(1.25f), o->Light);
			}
		}
		else if (o->SubType == 10)
		{
			...
			if (o->LifeTime < 40)
			{
				o->Light[0] /= ReduceWithDelta(1.1);
				o->Light[1] /= ReduceWithDelta(1.1);
				o->Light[2] /= ReduceWithDelta(1.1);
			}
			else if (o->LifeTime > 68)
			{
				o->Light[0] *= ReduceWithDelta(1.2);
				o->Light[1] *= ReduceWithDelta(1.2);
				o->Light[2] *= ReduceWithDelta(1.2);
			}
		}
		else if (o->SubType == 14)
		{
	...
	case BITMAP_2LINE_GHOST:
	{
		if (o->SubType == 0)
		{
			...
			{
				o->m_sTargetIndex = 10;
				o->Angle[2] += AdvanceByDelta(o->m_sTargetIndex);
			}
			else
			{
				o->m_sTargetIndex = -10;
	...
	case BITMAP_JOINT_SPIRIT2:
		if (o->SubType == 0 || o->SubType == 5 || o->SubType == 19)
		{
			if (o->Scale == 80.f)
			{
				...
				if (battleCastle::IsBattleCastleStart())
				{
					...
					{
						o->Velocity = 0.f;
						o->LifeTime /= ReduceWithDelta(5.f);
						o->life_time_work /= 5;
						break;
					}
				}
			...
			if (o->SubType != 5)
			{
				...
				o->Direction[0] += randuin * (0.2f) * AdvanceByDelta(1.0);
				o->Direction[2] += randuin * (0.8f) * AdvanceByDelta(1.0);
				o->Angle[0] += AdvanceByDelta(o->Direction[0]);
				o->Angle[2] += AdvanceByDelta(o->Direction[2]);
				o->Direction[0] *= ReduceWithDelta(0.6f);
		...
		else if (o->SubType == 3 || o->SubType == 13)
		{
			...
			else if (o->Target != NULL)
			{
				...
				if (o->SubType == 13)
				{
					...
					Position[0] += (AdvanceByDelta(rand() % 150 - 70.f));
					Position[1] += (AdvanceByDelta(rand() % 150 - 70.f));
					Position[2] += (AdvanceByDelta(50.f));
		...
		else if (8 == o->SubType)
		{
			if (o->LifeTime < 42)
			{
				...
				if (o->Angle[0] <= 180.0f)
					o->Angle[0] = 180.0f;
				if (o->Angle[0] <= 360.0f)
					o->Angle[0] += AdvanceByDelta(rand() % 3 + 2.0f);
			}
			else
			{
				o->Velocity = 10.f;
		...
		{
			o->Velocity = 50.f;
			o->Angle[2] -= AdvanceByDelta(50.0f);
		}
		else if (o->SubType == 9)
		{
			...
			if (o->LifeTime < 10)
			{
				o->Light[0] /= ReduceWithDelta(1.15f);
				o->Light[1] /= ReduceWithDelta(1.15f);
				o->Light[2] /= ReduceWithDelta(1.15f);
			}
		}
		else if (o->SubType == 11)
		{
			...
			o->Position[0] += AdvanceByDelta(cosf(o->Angle[2]) * 20.0f);
			o->Position[1] += AdvanceByDelta(sinf(o->Angle[2]) * 20.0f);
		}
		else if (o->SubType == 12)
		{
			Vector(1, 1, 1, Light);
		...
		else if (o->SubType == 15)
		{
			...
			if (o->LifeTime < 35)
			{
				...
				if (rand() % 2 == 0)
				{
					Position[0] -= (AdvanceByDelta(sinf(o->Angle[2]) * (40.0f - o->PKKey * 0.1f)));
					Position[1] += (AdvanceByDelta(cosf(o->Angle[2]) * (40.0f - o->PKKey * 0.1f)));
					Position[2] = 350;
					vec3_t Angle;
				...
				if (o->Target != NULL)
				{
					...
					VectorCopy(o->Position, Position);
					Position[0] += AdvanceByDelta(sinf(o->Angle[2]) * (30.0f + o->PKKey * 0.1f));
					Position[1] -= AdvanceByDelta(cosf(o->Angle[2]) * (30.0f + o->PKKey * 0.1f));
					//Position[2] = 350;
			...
			o->Position[0] += AdvanceByDelta(sinf(o->Angle[2]) * (30.0f + o->PKKey * 0.1f));
			o->Position[1] -= AdvanceByDelta(cosf(o->Angle[2]) * (30.0f + o->PKKey * 0.1f));
		}
		else if (o->SubType == 16)
		{
			Vector(1, 1, 1, Light);
			...
			o->Position[0] += AdvanceByDelta(cosf(o->Angle[2]) * 30.0f);
			o->Position[1] += AdvanceByDelta(sinf(o->Angle[2]) * 30.0f);
		}
		else if (o->SubType == 17)
		{
			Vector(1, 1, 1, Light);
		...
		{
			o->Velocity += AdvanceByDelta(3.0f);
			o->Angle[0] = -90.0f;

			size_t add = (size_t)(o->life_time_work % standlimit(12));

			if (add <= standlimit(5))
				o->PKKey += AdvanceByDelta(6);
			else
				o->PKKey -= AdvanceByDelta(6);

			o->Angle[0] -= AdvanceByDelta(o->PKKey);
			o->Angle[2] += AdvanceByDelta(10);
		...
		else if (o->SubType == 24)
		{
			...
			o->Direction[0] += AdvanceByDelta((float)(rand() % 32 - 16) * (0.2f));
			o->Direction[2] += AdvanceByDelta((float)(rand() % 32 - 16) * (0.8f));
			o->Angle[0] += AdvanceByDelta(o->Direction[0]);
			o->Angle[2] += AdvanceByDelta(o->Direction[2]);
			o->Direction[0] *= ReduceWithDelta(0.6f);
			o->Direction[2] *= ReduceWithDelta(0.8f);
		...
		else if (o->SubType == 25)
		{
			if (o->LifeTime < 10)
			{
				o->Light[0] /= (ReduceWithDelta(1.45f));
				o->Light[1] /= (ReduceWithDelta(1.45f));
				o->Light[2] /= (ReduceWithDelta(1.45f));
			...
			{
				o->Position[0] += AdvanceByDelta(rand() % 16 - 8.f);
				o->Position[1] += AdvanceByDelta(rand() % 16 - 8.f);
				o->Position[2] += AdvanceByDelta(o->Velocity);
				o->Scale += (AdvanceByDelta(3.0f));
			}
	...
	case MODEL_SPEARSKILL:
		...
		else if (o->SubType == 3)
		{
			...
			o->Position[0] += AdvanceByDelta(o->Direction[0]);
		...
		else if (o->SubType == 5 || o->SubType == 6 || o->SubType == 7)
		{
			for (int i = 0; i < 3; ++i)
			{
				...
				if (o->Weapon == 0)
				{
					o->Direction[1] *= ReduceWithDelta(0.95f);
					if (o->Direction[1] < 10.f)
						o->Direction[1] = -10.f;
				}
				if (o->Weapon > 40)
				{
					o->Direction[1] -= AdvanceByDelta(20.f);
					o->Angle[2] -= AdvanceByDelta(5.f);
					o->Scale += AdvanceByDelta(15.f);
				}
				if (o->Direction[1] < 0)
				{
	...
	case BITMAP_JOINT_LASER + 1:
		...
		for (int j = 0; j < o->MaxTails; j++)
		{
			...
			o->Direction[0] += AdvanceByDelta((float)(rand() % 256 - 128) / o->Scale);
			o->Direction[2] += AdvanceByDelta((float)(rand() % 256 - 128) / o->Scale);
			VectorScale(o->Direction, ReduceWithDelta(0.8f), o->Direction);
	...
	case BITMAP_JOINT_THUNDER + 1:
		if (o->SubType == 0)
		{
			...
			{
				o->Light[2] -= AdvanceByDelta(10.12f);
			}
		}
	...
	case BITMAP_SPARK + 1:
		if (o->SubType == 0)
		{
			o->Direction[2] += AdvanceByDelta(5.f);
	...
	case BITMAP_FLARE_BLUE:
		if (o->SubType == 0 || o->SubType == 10 || o->SubType == 18)
		{
			...
			{
				o->Position[2] += (AdvanceByDelta(o->Direction[2] * 1.1f));
				o->Velocity += (AdvanceByDelta(0.1f));
		...
		else if (o->SubType == 14 || o->SubType == 15)
		{
			if (o->Target != NULL)
			{
				...
				o->Scale += (AdvanceByDelta(1));
				o->Direction[1] += (AdvanceByDelta(4));
				o->Direction[2] += (AdvanceByDelta((rand() % 200 + 200) / 100.f));
			}
		}
		else if (o->SubType == 2 || o->SubType == 24 || o->SubType == 50 || o->SubType == 51)
		{
			if (o->SubType == 51)
			{
				o->Direction[2] += (AdvanceByDelta(10.f));
				o->Position[0] = o->TargetPosition[0];
				o->Position[1] = o->TargetPosition[1];
				o->Position[2] += AdvanceByDelta(o->Direction[2]);
			}
		...
		else if (o->SubType == 40)
		{
			...
			o->Position[0] += (AdvanceByDelta(o->Direction[0] * 12));
			o->Position[1] += (AdvanceByDelta(o->Direction[1] * 12));
			VectorCopy(o->Position, o->TargetPosition);
		...
		else if (o->SubType == 4 || o->SubType == 6 || o->SubType == 12)
		{
			...
			vPos[2] = 0.0f;
			o->TargetPosition[0] += AdvanceByDelta(o->Direction[1]);
			o->TargetPosition[1] += AdvanceByDelta(o->Direction[2]);
		...
		{
			o->Direction[2] -= AdvanceByDelta(60.f);
			o->Position[0] = o->TargetPosition[0];
			o->Position[1] = o->TargetPosition[1];
			o->Position[2] += AdvanceByDelta(o->Direction[2]);
			o->Scale = rand() % 4 + 6.f;
		...
		else if (o->SubType == 8)
		{
			...
			o->TargetPosition[2] += AdvanceByDelta(10.f);
		}
		else if (o->SubType == 9)
		{
			AngleMatrix(o->Angle, Matrix);
		...
		else if (o->SubType == 17)
		{
			...
			{
				o->LifeTime -= (AdvanceByDelta(10));
			}
			else
			{
				o->Direction[0] += AdvanceByDelta(sinf(rand() % 360 * 0.1f) * 4.f);
				o->Direction[1] += AdvanceByDelta(cosf(rand() % 360 * 0.1f) * 4.f);
				o->Direction[2] += AdvanceByDelta(o->Velocity);
		...
		else if (o->SubType == 23)
		{
			if (o->LifeTime < 20)
			{
				...
				else
				{
					...
					{
						o->Direction[1] += AdvanceByDelta(1.5f);
						switch ((int)o->MultiUse)
						{
						case 0:
							o->HeadAngle[0] += AdvanceByDelta(3.f);
							o->HeadAngle[2] -= AdvanceByDelta(o->Direction[1]);
							break;
						case 1:
							o->HeadAngle[0] -= AdvanceByDelta(3.f);
							o->HeadAngle[2] -= AdvanceByDelta(o->Direction[1]);
						...
						case 4:
							...
							break;
						case 5:
							o->HeadAngle[0] -= AdvanceByDelta(o->Direction[1]);
							break;
						}
					...
					o->Direction[2] += AdvanceByDelta(4.f);
				}
			}
		}
		else if (o->SubType == 43)
		{
			if (o->Target->Live)
			{
				...
				{
					VectorCopy(o->Target->Position, o->Position);
					o->Position[2] += AdvanceByDelta(30.f);
					o->Position[0] += AdvanceByDelta((rand() % 60 - 30));
					o->Position[1] += AdvanceByDelta((rand() % 60 - 30));
		...
		else if (o->SubType == 44)
		{
			...
			else
			{
				if (o->LifeTime <= 20)
				{
					o->Direction[2] += AdvanceByDelta(1.0f);
					o->Position[0] = o->TargetPosition[0];
					o->Position[1] = o->TargetPosition[1];
					o->Position[2] += (o->Direction[2]);
	...
	case BITMAP_FLARE + 1:
		if (o->SubType == 5)
		{
			...
			{
				o->Angle[2] -= AdvanceByDelta(10.f);
				o->Position[2] += AdvanceByDelta(2.5f);
			}
			else
			{
				o->Velocity += AdvanceByDelta(5.f);
		...
		else if (o->SubType == 6)
		{
			if (o->Target != NULL)
			{
				...
				float fOldAngle = o->Angle[2];
				VectorCopy(o->Target->Position, Position);
				Position[2] += AdvanceByDelta(150.f);
				Distance = MoveHumming(o->Position, o->Angle, Position, o->Velocity);
		...
		else if (o->SubType == 7)
		{
			...
			o->Position[2] += AdvanceByDelta(o->Velocity);
			o->Velocity -= AdvanceByDelta(0.1f);
			o->Direction[0] -= AdvanceByDelta(0.1f);
		...
		else if (o->SubType == 8 || o->SubType == 9)
		{
			...
			{
				o->Position[2] += AdvanceByDelta(o->MultiUse);
			}
		}
		...
		else if (o->SubType == 19)
		{
			...
			pos[0] /= ReduceWithDelta(3.f);
			pos[1] /= ReduceWithDelta(3.f);
			pos[2] /= ReduceWithDelta(3.f);
		...
		else
		{
			...
			if (o->Target->Live)
			{
				...
				if (o->LifeTime < 15)
				{
					o->Light[0] /= ReduceWithDelta(1.5f);//o->LifeTime/60.f;
	...
	case BITMAP_JOINT_FORCE:
		...
		else if (o->SubType == 1)
		{
			...
			o->Position[2] += AdvanceByDelta(100.f);
		...
		else if (o->SubType == 1)
		{
			if (o->Skill == 0 || o->LifeTime > 3)
			{
				...
				for (int i = 0; i < 5; ++i)
				{
					...
					VectorRotate(p, Matrix, vPos);
					interpolatesum(o->Position, vPos);
					o->Velocity += AdvanceByDelta(0.5f);
	...
	case BITMAP_FLASH:
		if (o->SubType <= 3 || o->SubType == 5)
		{
			if (o->SubType == 2)
			{
				o->Angle[2] += AdvanceByDelta(rand() % 10 + 10.f);
			}
		...
		{
			int fr = rand() % 5 - 2;
			o->Direction[1] += AdvanceByDelta(fr);
			o->Direction[2] -= AdvanceByDelta(10.f);
	...
	case BITMAP_GROUND_WIND:
	{
		...
		if (pModel)
		{
			...
			o->Position[2] += AdvanceByDelta(20.0f);
	...
	for (int i = 0; i < MAX_JOINTS; i++)
	{
		...
		if (o->Live && o->NumTails > 0 && o->RenderFace != 0)
		{
			...
			if (o->Type == BITMAP_JOINT_THUNDER + 1 && o->SubType == 6)
			{
				vec3_t Light;
				EnableAlphaBlend();
				o->Velocity /= (ReduceWithDelta(1.1f));
