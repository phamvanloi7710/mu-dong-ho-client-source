...
{
	Position[0] += AdvanceByDelta((float)(rand() % 16 - 8));
	Position[1] += AdvanceByDelta((float)(rand() % 16 - 8));
	Position[2] += AdvanceByDelta((float)(rand() % 16 - 8));
	vec3_t Light;
...
void CheckSkull(OBJECT* o)
{
	...
	VectorScale(o->Direction, ReduceWithDelta(0.6f), o->Direction);
	VectorScale(o->HeadAngle, ReduceWithDelta(0.6f), o->HeadAngle);
...
bool MoveHeavenRain(PARTICLE* o)
{
	...
	else
	{
		...
		o->Velocity[0] += AdvanceByDelta((float)(rand() % 16 - 8) * 0.1f);
		o->Velocity[1] += AdvanceByDelta((float)(rand() % 16 - 8) * 0.1f);
		o->Velocity[2] += AdvanceByDelta((float)(rand() % 16 - 8) * 0.1f);

		interpolatesum2(o->Position, o->Velocity, o->Position);

		o->TurningForce[0] += AdvanceByDelta(((float)(rand() % 8 - 4) * 0.02f));
		o->TurningForce[1] += AdvanceByDelta(((float)(rand() % 16 - 8) * 0.02f));
		o->TurningForce[2] += AdvanceByDelta(((float)(rand() % 8 - 4) * 0.02f));
