...
bool MoveJump(OBJECT* o)
{
	...
	{
		o->Position[2] = Height;
		o->Gravity = -o->Gravity * ReduceWithDelta(0.4f);
		o->Direction[1] *= ReduceWithDelta(0.4f);
		return true;
	}
	return false;
}
...
void CheckClientArrow(OBJECT* o)
{
	if (battleCastle::IsBattleCastleStart())
	{
		...
		if ((att & TW_NOATTACKZONE) == TW_NOATTACKZONE)
		{
			o->Velocity = 0.f;
			Vector(0.f, 0.f, 0.f, o->Direction);
			o->LifeTime /= ReduceWithDelta(2.f);
...
void MoveEffect(OBJECT* o, int iIndex)
{
	...
	case MODEL_DRAGON:
	{
		...
		if (fTemp <= 800.0f && fTemp >= -800.0f)
		{
			o->Kind = 2;
			o->Distance = 0.0f;
			o->Angle[2] += (0.5f * o->Distance / 2.0f) * AdvanceByDelta(1.0);
			o->CollisionRange = true;
		}
		else
		{
			...
			{
				o->Angle[2] += (0.5f * o->Gravity * 3.0f) * AdvanceByDelta(1.0);
				o->Kind = 1;
			}
	...
	case MODEL_ARROW_AUTOLOAD:
	{
		...
		else if (o->SubType == 1)
		{
			...
			o->Angle[0] += AdvanceByDelta(85.f);
			o->Angle[1] += AdvanceByDelta(-17.f);
			o->Angle[2] += AdvanceByDelta(20.f);
		}
	}
	break;
	case MODEL_INFINITY_ARROW:
	...
	case MODEL_INFINITY_ARROW3:
	{
		...
		{
			o->Light[0] *= ReduceWithDelta(0.98f);
			o->Light[1] *= ReduceWithDelta(0.98f);
			o->Light[2] *= ReduceWithDelta(0.98f);
			if (o->LifeTime == (40))
	...
	case MODEL_IRON_RIDER_ARROW:
	{
		...
		vec3_t vPos;
		VectorScale(o->Direction, ReduceWithDelta(o->Velocity), vPos);
	...
	case MODEL_GHOST:
	{
		if (rand() % 40 == 1)
		{
			o->Velocity *= -ReduceWithDelta(1.0f);
	...
	{
		if (o->Kind > 0)
			o->Angle[2] += AdvanceByDelta((rand() % 10));
		else
			o->Angle[2] -= AdvanceByDelta((rand() % 10));
		...
		o->Direction[2] += AdvanceByDelta(((float)(rand() % 15 - 7) * 0.2f));

		float Height = RequestTerrainHeight(o->Position[0], o->Position[1]);

		if (o->Position[2] < Height + 50.f)
		{
			o->Direction[2] *= ReduceWithDelta(0.8f);
			o->Direction[2] += AdvanceByDelta(1.f);
		}
		...
		o->Position[2] += AdvanceByDelta(((float)(rand() % 15 - 7) * 0.3f));
	...
	case BITMAP_SKULL:
		if (o->SubType == 4)
		{
			o->Direction[1] -= AdvanceByDelta(o->Velocity);
			o->Angle[0] -= AdvanceByDelta(1.f);
	...
	case MODEL_HALLOWEEN_CANDY_STAR:
		if (o->SubType == 0 || o->SubType == 1)
		{
			o->Angle[o->m_iAnimation] += AdvanceByDelta(20.f);
	...
	case MODEL_NEWYEARSDAY_EVENT_PIG:
	case MODEL_NEWYEARSDAY_EVENT_YUT:
		o->Angle[o->m_iAnimation] += AdvanceByDelta(10.f);
		o->Position[0] += AdvanceByDelta((o->Direction[0] * 1.2f));
		o->Position[1] += AdvanceByDelta((o->Direction[1] * 1.2f));
		...
		{
			o->Position[2] = Height;
			o->Gravity = -o->Gravity * ReduceWithDelta(0.3f);
			o->LifeTime -= AdvanceByDelta(2);
		}
		VectorAdd(o->Position, o->Direction, o->Position);
		break;
	case MODEL_MOONHARVEST_MOON:
		if (o->SubType == 0)
		{
			o->Angle[2] += AdvanceByDelta(5.0f);
			if (o->LifeTime < 25)
			{
				o->Light[0] -= AdvanceByDelta(0.05f);
				o->Light[1] -= AdvanceByDelta(0.06f);
	...
	case MODEL_MOONHARVEST_SONGPUEN1:
	case MODEL_MOONHARVEST_SONGPUEN2:
		o->Angle[0] += AdvanceByDelta(10.f);
		o->Position[0] += AdvanceByDelta((o->Direction[0] * 2.2f));
		o->Position[1] += AdvanceByDelta((o->Direction[1] * 2.2f));
		o->Position[2] += (o->Gravity * 1.5f);
		o->Gravity -= AdvanceByDelta(1.5f);
	...
	{
		if (o->LifeTime < 20)
			o->BlendMeshLight -= AdvanceByDelta(0.03f);
		else if (o->BlendMeshLight < 0.5f)
			o->BlendMeshLight += AdvanceByDelta(0.05f);

		switch (o->Type)
		{
		case MODEL_SUMMONER_CASTING_EFFECT1:
			o->Angle[2] -= AdvanceByDelta(3.0f);
			break;
		case MODEL_SUMMONER_CASTING_EFFECT11:
			o->Angle[2] += AdvanceByDelta(3.0f);
			break;
	...
	break;
	{
		...
		o->BlendMeshTexCoordV += AdvanceByDelta(0.05f);
	}
	break;
	case MODEL_SUMMONER_SUMMON_SAHAMUTT:
	{
	...
	case BITMAP_MAGIC:
		...
		else if (o->SubType == 9)
		{
			...
			o->HeadAngle[0] += AdvanceByDelta(4.0f);
			o->HeadAngle[1] -= AdvanceByDelta(8.0f);
			o->HeadAngle[2] += AdvanceByDelta(4.0f);
		}
		else if (o->SubType == 10)
		{
			if (o->LifeTime < 20)
		...
		{
			o->HeadAngle[0] += AdvanceByDelta(2.0f);
			o->HeadAngle[1] -= AdvanceByDelta(2.0f);
			o->HeadAngle[2] += AdvanceByDelta(2.0f);
		...
		else if (o->SubType == 12)
		{
			...
			o->Scale += AdvanceByDelta(o->Alpha);
			if (o->Scale > 5.0f)
			{
		...
		else if (o->SubType == 13)
		{
			...
			if (o->Scale > 4)
			{
				VectorScale(o->Light, ReduceWithDelta(0.5f), o->Light);
			}
		}
		else if (o->SubType == 14)
		{
	...
	case BITMAP_ENEMY_INFLUENCE_GROUND:
		if (o->SubType == 0)
		{
			...
			if (o->LifeTime < 25)
				o->AlphaTarget -= AdvanceByDelta(0.02f);
	...
	case BITMAP_IMPACT:
	{
		...
		if (o->LifeTime <= 40)
		{
			...
			o->BlendMeshLight -= AdvanceByDelta(0.1f);
		}
		float Matrix[3][4];
		...
		o->Position[2] += AdvanceByDelta(130);
	...
	case MODEL_ALICE_BUFFSKILL_EFFECT2:
	{
		if (o->SubType == 0 || o->SubType == 1 || o->SubType == 2)
		{
			...
			VectorCopy(o->Owner->Position, o->Position);
			o->Position[2] += AdvanceByDelta(100);
			if (o->LifeTime > 20)
			{
				o->Alpha += AdvanceByDelta(0.05f);
				o->BlendMeshLight += AdvanceByDelta(0.05f);
	...
	case MODEL_LIGHTNING_SHOCK:
	{
		...
		if (o->SubType == 0)
		{
			if (o->Owner->AnimationFrame > 6.0f || o->LifeTime < 15)
			{
				...
				o->Velocity += AdvanceByDelta(o->Gravity);
			}
	...
	case BITMAP_LIGHT:
		...
		{
			VectorAdd(o->Position, o->Direction, o->Position);
			o->Direction[2] -= AdvanceByDelta(0.01f);
	...
	case MODEL_WAVE:
		...
		{
			o->Scale += AdvanceByDelta(0.1f);
			o->Position[0] -= AdvanceByDelta(1.f);
			o->Position[2] -= AdvanceByDelta(1.5f);
			o->BlendMeshLight = o->LifeTime._runvalue / 30.f;
		}
		else
		{
			o->Scale += AdvanceByDelta(1.2f);
			o->Position[0] -= AdvanceByDelta(1.2f);
			o->Position[2] -= AdvanceByDelta(1.8f);
		}
	...
	case MODEL_SKILL_INFERNO:
		...
		else if (o->SubType == 8)
		{
			...
			if (o->LifeTime < 10)
			{
				o->Light[0] *= (ReduceWithDelta(0.8f));
				o->Light[1] *= (ReduceWithDelta(0.8f));
				o->Light[2] *= (ReduceWithDelta(0.8f));
			}
		}
		else if (o->SubType == 3)
		{
		...
		else if (o->SubType == 4)
		{
			...
			o->Gravity += AdvanceByDelta(0.8f * o->Scale * 30.f);
			o->Position[2] += AdvanceByDelta(o->Gravity);
		}
		else if (o->SubType == 5)
		{
	...
	case MODEL_BIG_METEO3:
		...
		VectorAdd(o->Position, o->Direction, o->Position);
		o->Angle[0] += AdvanceByDelta(10.f / o->Scale);
	...
	case MODEL_SKILL_WHEEL2:
		...
		o->Angle[2] -= (18) * AdvanceByDelta(1.0);//
		...
		case 0:
			...
			if (battleCastle::IsBattleCastleStart())
			{
				...
				if ((att & TW_NOATTACKZONE) == TW_NOATTACKZONE)
				{
					o->Velocity = 0.f;
					Vector(0.f, 0.f, 0.f, o->Direction);
					o->LifeTime._runvalue /= ReduceWithDelta(5.f);
	...
	case MODEL_STORM2:
	{
		if (o->SubType == 0)
		{
			o->Angle[1] += AdvanceByDelta(40.0f);
		...
		else if (o->SubType == 1)
		{
			...
			o->Angle[2] += AdvanceByDelta(o->Gravity);
	...
	case MODEL_STORM3:
	{
		...
		o->Angle[1] += AdvanceByDelta(30.0f);
	...
	case MODEL_MAYASTAR:
	{
		...
		o->Angle[1] += AdvanceByDelta(10.0f);
	...
	case MODEL_MAYASTONE5:
	{
		...
		o->Position[2] += (o->Gravity);
		o->Gravity -= AdvanceByDelta(3.f);
		Height = RequestTerrainHeight(o->Position[0], o->Position[1]);
		if (o->Position[2] < Height)
		{
			o->Position[2] = Height;
			o->Gravity = -o->Gravity * ReduceWithDelta(0.5f);
			o->LifeTime -= AdvanceByDelta(4);
			o->Angle[0] -= AdvanceByDelta(o->Scale * 128.f);
		}
		else
			o->Angle[0] -= AdvanceByDelta(o->Scale * 32.f);
	...
	case MODEL_MAYAHANDSKILL:
	{
		if (o->SubType == 0)
		{
			o->Light[0] /= ReduceWithDelta(1.17f);
			o->Light[1] /= ReduceWithDelta(1.17f);
			o->Light[2] /= ReduceWithDelta(1.17f);
	...
	case MODEL_STONE2:
		...
		{
			o->HeadAngle[2] -= (AdvanceByDelta(o->Gravity));
			...
			{
				o->Position[2] = Height;
				o->HeadAngle[0] *= (ReduceWithDelta(0.6f));
				o->HeadAngle[1] *= (ReduceWithDelta(0.6f));
	...
	case MODEL_EFFECT_SAPITRES_ATTACK_2:
		...
		else if (o->Type == MODEL_EFFECT_SAPITRES_ATTACK_2 && o->SubType == 14)
		{
			...
			o->Position[0] -= AdvanceByDelta((o->Direction[0] * fMoveSpeed));
			o->Position[1] -= AdvanceByDelta((o->Direction[1] * fMoveSpeed));
			o->Position[2] -= AdvanceByDelta((o->Direction[2] * fMoveSpeed));
	...
	case MODEL_NARCONDRA_STONE3:
		if (o->SubType == 0)
		{
			...
			o->Position[0] += AdvanceByDelta(o->HeadAngle[0]);
			o->Position[1] += AdvanceByDelta(o->HeadAngle[1]);
			o->Position[2] += (o->HeadAngle[2]);
		...
		break;
	case MODEL_SNOW1:
		o->Direction[2] -= AdvanceByDelta(2.5f);
		CheckTargetRange(o);
		break;
	case MODEL_WOOSISTONE:
		if (o->SubType == 1)
		...
		{
			o->Direction[2] -= AdvanceByDelta(2.0f);
			CheckTargetRange(o);
		}
		break;
	case MODEL_SKULL:
		if (o->SubType == 1 && o->Owner != NULL)
		{
			...
			o->HeadAngle[0] += ((float)(rand() % 32 - 16) * ReduceWithDelta(0.2f));
			o->HeadAngle[2] += ((float)(rand() % 32 - 16) * ReduceWithDelta(0.8f));
			o->Angle[0] += (o->HeadAngle[0]);
			o->Angle[2] += (o->HeadAngle[2]);
			o->HeadAngle[0] *= (ReduceWithDelta(0.6f));
			o->HeadAngle[2] *= (ReduceWithDelta(0.8f));

			if (o->LifeTime < 10)
			{
				o->Alpha /= (ReduceWithDelta(1.1f));
				o->BlendMeshLight /= (ReduceWithDelta(1.2f));

				if (o->Angle[0] <= 80)
				{
					o->Angle[0] += (AdvanceByDelta(5.f));
				}
			}
			else
			{
	...
	case MODEL_MONSTER01 + 64:
		...
		case 3:
			if (o->LifeTime < 250 + 50 + (34 - o->Skill) * 5)
			{
				if (o->Position[2] > 350)
				{
					...
					o->Direction[2] -= AdvanceByDelta(6.0f);
				}
				else
				{
		...
		case 4:
			if (o->LifeTime < 140)
			{
				...
				{
					o->Direction[2] -= AdvanceByDelta(3.5f);
				}
				else
				{
					o->Direction[0] = 0;
	...
	case MODEL_CURSEDTEMPLE_STATUE_PART2:
		...
		if (o->Position[2] > 290)
		{
			o->Direction[2] -= AdvanceByDelta(o->Gravity);
			o->Angle[2] -= AdvanceByDelta((float)(rand() % 10));
		}
	...
	case MODEL_XMAS2008_SNOWMAN_HEAD:
	{
		if (o->LifeTime < 45)
		{
			o->Position[0] += (o->Direction[0] * ReduceWithDelta(1.2f));
			o->Position[1] += (o->Direction[1] * ReduceWithDelta(1.2f));
			o->Position[2] += ((o->Gravity * 1.5f));
			o->Gravity -= AdvanceByDelta(1.5f);
	...
	case MODEL_NARCONDRA_NECK:
		...
		o->Position[0] += AdvanceByDelta(o->HeadAngle[0]);
		o->Position[1] += AdvanceByDelta(o->HeadAngle[1]);
		o->Position[2] += AdvanceByDelta(o->HeadAngle[2]);
		...
		{
			o->Position[2] = Height;
			o->HeadAngle[0] *= ReduceWithDelta(0.8f);
			o->HeadAngle[1] *= ReduceWithDelta(0.8f);
	...
	case MODEL_ICE_GIANT_PART5:				case MODEL_ICE_GIANT_PART6:
		...
		{
			o->Position[2] = Height;
			o->HeadAngle[0] *= ReduceWithDelta(0.5f);
			o->HeadAngle[1] *= ReduceWithDelta(0.5f);
			o->HeadAngle[2] += (0.6f * o->LifeTime._runvalue);
			if (o->HeadAngle[2] < 5.0f)
				o->HeadAngle[2] = 0;
	...
	case MODEL_WATER_WAVE:
		...
		o->Velocity -= AdvanceByDelta(10.f);
		o->Gravity -= AdvanceByDelta(20.f);
		if (o->Gravity < 60)
			o->Gravity = 60;
	...
	case MODEL_ARROW_BEST_CROSSBOW:
		...
		o->Angle[1] += AdvanceByDelta(rand() % 60 + 30.f);
		VectorCopy(o->Position, o->EyeLeft);
	...
	case MODEL_ARROW:
		...
		if (o->SubType == 3 || o->SubType == 4)
		{
			if (o->SubType == 3)
			{
				o->Angle[0] += AdvanceByDelta(o->Gravity);
			}
			if (o->Angle[0] > 50)
			{
				o->Angle[0] = 50.f;
	...
	case MODEL_ARROW_SPARK:
	{
		o->Angle[1] += AdvanceByDelta(35.0f);
		CheckClientArrow(o);
	}
	break;
	case MODEL_ARROW_RING:
	...
	case MODEL_DUNGEON_STONE01:
		...
		if (o->Position[2] < Height)
		{
			...
			o->Gravity = -o->Gravity * ReduceWithDelta(0.4f);
			o->LifeTime -= AdvanceByDelta(4);
			o->Direction[1] -= AdvanceByDelta(2.f);
		}
		break;
	case MODEL_ICE:
		switch (o->SubType)
		...
		case 2:
			if (o->Owner != NULL)
			{
				...
				if (o->SubType == 1)
				{
					o->HeadAngle[2] += AdvanceByDelta(20.f);
	...
	case MODEL_FIRE:
		...
		else if (o->SubType == 5)
		{
			...
			{
				o->Gravity = 10.f;
				o->Position[2] += AdvanceByDelta(10.f);
				o->Direction[1] *= ReduceWithDelta(0.5f);
				o->Scale *= ReduceWithDelta(1.1f);
			}
		...
		else
		{
			...
			else
			{
				...
				VectorScale(vRot, ReduceWithDelta(50.f), vRot);
	...
	case MODEL_GATE + 1:
		VectorAdd(o->Position, o->Direction, o->Position);
		VectorScale(o->Direction, ReduceWithDelta(0.9f), o->Direction);
		o->Position[2] += AdvanceByDelta(o->Gravity);

		if (o->SubType == 1)
		{
			VectorScale(o->Direction, ReduceWithDelta(1.1f), o->Direction);
		}
		...
		if (o->Position[2] < Height)
		{
			o->Position[2] = Height;
			o->Gravity = -o->Gravity * ReduceWithDelta(0.5f);
			o->LifeTime -= AdvanceByDelta(5);
			o->Angle[0] -= AdvanceByDelta(o->Scale * 128.f);
		}
		else
			o->Angle[0] -= AdvanceByDelta(o->Scale * 16.f);
	...
	case MODEL_STONE_COFFIN + 1:
		...
		{
			o->Position[2] = Height;
			o->Gravity = -o->Gravity * ReduceWithDelta(0.2f);
			o->LifeTime -= AdvanceByDelta(5);
			o->Angle[0] -= AdvanceByDelta(o->Scale * 128.f);
		}
		else
	...
	case MODEL_BLIZZARD:
		if (o->SubType == 0)
		{
			...
			o->Position[1] = o->StartPosition[1] + sinf((rand() % 1000) * 0.01f) * 10.f;
			o->Position[2] += AdvanceByDelta(o->Gravity);
			o->Gravity -= AdvanceByDelta(rand() % 5);
		...
		{
			o->BlendMeshLight /= ReduceWithDelta(1.1f);
	...
	case MODEL_COMBO:
		if (o->SubType == 0)
		{
			...
			o->BlendMeshLight /= ReduceWithDelta(1.4f);
		}
		break;
	case MODEL_WAVES:
		if (o->SubType == 0)
		...
		{
			o->Scale += AdvanceByDelta(o->Gravity);
			o->Gravity += AdvanceByDelta(0.07f);
			if (o->Scale > 2.f)
				o->Scale = 2.f;
			o->BlendMeshLight /= ReduceWithDelta(1.5f);
		}
		else if (o->SubType == 2)
		{
			o->Scale += AdvanceByDelta(o->Gravity);
		...
		{
			o->Scale += AdvanceByDelta(o->Gravity);
			o->Gravity += AdvanceByDelta(0.005f);
			if (o->Scale > 1.5f)
				o->Scale = 1.5f;
			o->BlendMeshLight /= ReduceWithDelta(1.3f);
			o->Angle[1] += AdvanceByDelta(45.f);
		}
		else if (o->SubType == 4)
		{
			o->Scale += AdvanceByDelta(o->Gravity);
			o->Gravity += AdvanceByDelta(0.002f);
			if (o->Scale > 2.5f)
				o->Scale = 2.5f;
			o->BlendMeshLight /= ReduceWithDelta(1.2f);
			o->Angle[1] += AdvanceByDelta(45.f);
		...
		{
			o->Scale += AdvanceByDelta(o->Gravity);
			o->Gravity += AdvanceByDelta(0.015f);
			if (o->Scale > 2.5f)
				o->Scale = 2.5f;
			o->BlendMeshLight /= ReduceWithDelta(1.3f);
			o->Angle[1] += AdvanceByDelta(45.f);
	...
	case MODEL_PIERCING2:
		...
		o->Direction[1] += AdvanceByDelta(12.f);
		if (o->Direction[1] >= 0)
			o->Direction[1] = 0;
		...
		if (o->SubType == 1)
		{
			if (o->LifeTime > 1)
			{
				o->BlendMeshLight /= ReduceWithDelta(1.6f);
		...
		if (o->SubType == 0)
		{
			if (o->Owner != NULL)
			{
				...
				for (int i = 1; i < o->Gravity; ++i)
				{
					...
					{
						if (o->Angle[0] < -90)
							o->Angle[0] += AdvanceByDelta(20.f);
						else
							o->Angle[0] -= AdvanceByDelta(20.f);
					}
					Distance = MoveHumming(o->Position, o->Angle, p, o->Velocity);
					o->Velocity += AdvanceByDelta(0.4f);

					if (o->LifeTime < 10)
					{
						o->Velocity += AdvanceByDelta(0.1f);
	...
	case MODEL_PIER_PART:
		...
		else if (o->SubType == 2)
		{
			if (o->Owner != NULL)
			{
				...
				o->Velocity += AdvanceByDelta(2.4f);
	...
	case MODEL_DARKLORD_SKILL:
		if (o->SubType <= 1)
		{
			o->Scale += AdvanceByDelta(o->Velocity);
			o->Velocity += AdvanceByDelta(0.02f);
			if (o->LifeTime < 7)
				o->BlendMeshLight /= (ReduceWithDelta(1.8f));
		}
		else if (o->SubType == 2)
		{
			o->Scale += AdvanceByDelta(o->Velocity);
			o->Velocity -= AdvanceByDelta(0.06f);
			if (o->LifeTime < 8)
				o->BlendMeshLight /= (ReduceWithDelta(1.5f));
		}
		else
		{
			o->Scale += AdvanceByDelta(o->Velocity);
			o->Velocity += AdvanceByDelta(0.04f);
			if (o->LifeTime < 8)
				o->BlendMeshLight /= (ReduceWithDelta(1.8f));
		}
		break;
	...
	case MODEL_GROUND_STONE2:
		...
		{
			o->Alpha /= (ReduceWithDelta(1.3f));
			o->BlendMeshLight /= (ReduceWithDelta(2.f));
		}
	...
	case BITMAP_TWLIGHT:
	{
		...
		{
			o->Scale -= AdvanceByDelta(0.1f);
			o->Angle[2] += AdvanceByDelta(5.f);
		}
		else if (o->SubType == 2)
		{
			o->Scale -= AdvanceByDelta(0.1f);
			o->Angle[2] += AdvanceByDelta(15.f);
		}
		else if (o->SubType == 3)
		{
			VectorCopy(o->Owner->Position, o->Position);
			...
			if (o->LifeTime >= 20)
			{
				o->PKKey++;
				o->Alpha += AdvanceByDelta(0.1f);
				o->Light[0] = o->EyeRight[0] * (o->PKKey * ReduceWithDelta(0.1f));
				o->Light[1] = o->EyeRight[1] * (o->PKKey * ReduceWithDelta(0.1f));
				o->Light[2] = o->EyeRight[2] * (o->PKKey * ReduceWithDelta(0.1f));
			}
			else if (o->LifeTime <= 10)
			{
				o->PKKey--;
		...
		{
			o->Angle[2] -= AdvanceByDelta(60.0);
			o->Scale += AdvanceByDelta(0.12f);
			o->Position[2] = 130.0;

			if (o->LifeTime <= 18)
			{
				o->Light[0] *= ReduceWithDelta(0.75);
				o->Light[1] *= ReduceWithDelta(0.75);
				o->Light[2] *= ReduceWithDelta(0.75);
			}
			else
			{
				o->Light[0] *= ReduceWithDelta(1.05);
				o->Light[1] *= ReduceWithDelta(1.05);
				o->Light[2] *= ReduceWithDelta(1.05);
			}
		}
		else if (o->SubType == 5)
		{
	...
	case BITMAP_SHOCK_WAVE:
		if (o->SubType == 0)
		{
			o->Scale -= AdvanceByDelta(1.f);
		}
		else if (o->SubType == 1)
		{
			o->Scale += (AdvanceByDelta((rand() % 5) / 10.f));

			o->Position[0] += AdvanceByDelta((rand() % 8 - 4.f));
			o->Position[1] += AdvanceByDelta((rand() % 8 - 4.f));
		}
		else if (o->SubType == 2)
		{
			o->Scale += (AdvanceByDelta((rand() % 5) / 40.f));
		...
		{
			if (o->LifeTime > 4.0f)
				o->Scale += (AdvanceByDelta((o->LifeTime._runvalue - 4.0f) * 0.25f));
		}
		else if (o->SubType == 13)
		{
			o->Scale += (AdvanceByDelta(0.08f));
	...
	case MODEL_CUNDUN_DRAGON_HEAD:
		o->Angle[2] += AdvanceByDelta(10 + rand() % 10);
	...
	case MODEL_CUNDUN_GHOST:
		...
		{
			o->PKKey += (1);
			o->Position[2] += ((o->PKKey * ReduceWithDelta(0.8f)));
		...
		else
		{
			o->Skill += (1);
			if (o->Angle[2] < 45.0f)
				o->Angle[2] += (o->Skill * ReduceWithDelta(0.3f));
			if (o->Angle[2] > 45.0f)
				o->Angle[2] -= (o->Skill * ReduceWithDelta(0.3f));

			if (o->Position[0] < Hero->Object.Position[0] + 300.0f)
				o->Position[0] += (o->Skill * ReduceWithDelta(0.2f));
			if (o->Position[0] > Hero->Object.Position[0] + 300.0f)
				o->Position[0] -= (o->Skill * ReduceWithDelta(0.2f));
			if (o->Position[1] < Hero->Object.Position[1] - 300.0f)
				o->Position[1] += (o->Skill * ReduceWithDelta(0.2f));
			if (o->Position[1] > Hero->Object.Position[1] - 300.0f)
				o->Position[1] -= (o->Skill * ReduceWithDelta(0.2f));
	...
	case MODEL_ARROW_TANKER_HIT:
	{
		...
		if (o->Position[2] < Height + 80.0f)
		{
			...
			EarthQuake = (float)(rand() % 8 - 8) * 0.1f;
			EarthQuake /= ReduceWithDelta(2.f);
		...
		if (o->HiddenMesh == 99)
		{
			o->Position[0] += AdvanceByDelta((float)(rand() % 100 - 50));
			o->Position[1] += AdvanceByDelta((float)(rand() % 100 - 50));
			o->Position[2] += AdvanceByDelta((float)(rand() % 100 - 50));
			o->Scale = 2.2f;
		}
	...
	case MODEL_FLY_BIG_STONE1:
	{
		...
		{
			o->Position[2] -= (o->Gravity);
			o->Gravity += AdvanceByDelta(1.9f);

			o->Angle[0] += AdvanceByDelta(rand() % 20 + 20.f);
			o->Angle[1] += AdvanceByDelta(rand() % 5);
		...
		else
		{
			if (o->SubType <= 1)
			{
				o->Position[0] += AdvanceByDelta(o->Direction[0] * o->Velocity);
				o->Position[1] += AdvanceByDelta(o->Direction[1] * o->Velocity);
				o->StartPosition[2] += AdvanceByDelta(o->Direction[2]);
				...
				o->Velocity -= AdvanceByDelta(0.45f);
				if (o->Velocity < 5.f)
					o->Velocity = 5.f;
	...
	case MODEL_GOLEM_STONE:
		if (o->Type == MODEL_BIG_STONE_PART2 && o->SubType == 3)
		{
			...
			o->Velocity += AdvanceByDelta(0.3f);
	...
	case MODEL_FENRIR_THUNDER:
		if (o->SubType == 0)
		{
			if (o->Live)
			{
				...
				o->Angle[0] += AdvanceByDelta(0.15f);
				o->Angle[1] += AdvanceByDelta(0.15f);
				o->Angle[2] += AdvanceByDelta(0.15f);
			}
		}
		else if (o->SubType == 1)
		{
	...
	case MODEL_FALL_STONE_EFFECT:
	{
		if (o->SubType == 0 || o->SubType == 1)
		{
			o->Position[2] -= (o->Gravity);
			o->Gravity += AdvanceByDelta(0.1f);
			o->Angle[0] += AdvanceByDelta(0.5f);
			o->Angle[1] += AdvanceByDelta(0.5f);
			o->Angle[2] += AdvanceByDelta(0.5f);
		}
		else if (o->SubType == 2)
		{
			o->Position[2] -= (o->Gravity);
	...
	case MODEL_STUN_STONE:
		if (o->SubType == 0)
		{
			...
			else
			{
				...
				VectorScale(o->StartPosition, ReduceWithDelta(0.9f), o->StartPosition);

				o->HeadAngle[0] -= AdvanceByDelta(o->Scale * 32.f);
	...
	case MODEL_MANA_RUNE:
		if (o->SubType == 0)
		{
			...
			{
				o->Scale += AdvanceByDelta(o->Gravity);
				o->Gravity += AdvanceByDelta(0.15f);
				if (o->Scale > 1.f)
				{
					o->Scale = 1.f;
					o->Gravity = 0.01f;
	...
	case MODEL_SKILL_JAVELIN:
		...
		{
			VectorCopy(o->Owner->Position, o->StartPosition);
			o->StartPosition[2] += AdvanceByDelta(150.f);
		}
		...
		if (o->LifeTime < 25)
		{
			...
			o->Velocity += AdvanceByDelta(1.5f);
			...
			{
				o->Direction[1] -= AdvanceByDelta(8.f);
			}
		}
		else
		{
			o->Gravity += AdvanceByDelta(10.f);
			AngleMatrix(o->HeadAngle, Matrix);
			VectorRotate(o->Direction, Matrix, Position);
			interpolatesum2(o->Position, Position, o->Position);
		}
	...
	case BITMAP_JOINT_FORCE:
		if (o->SubType == 0)
		{
			...
			{
				Vector(90.f, 0.f, 0.f, o->Angle);
				o->HeadAngle[2] += AdvanceByDelta(72.f);
				AngleMatrix(o->HeadAngle, Matrix);
				VectorRotate(o->Direction, Matrix, Position);
	...
	case MODEL_MOVE_TARGETPOSITION_EFFECT:
	{
		if (o->SubType == 0)
		{
			...
			{
				o->BlendMeshLight -= (AdvanceByDelta(0.05f));
				o->Light[0] *= (ReduceWithDelta(o->BlendMeshLight));
				o->Light[1] *= (ReduceWithDelta(o->BlendMeshLight));
				o->Light[2] *= (ReduceWithDelta(o->BlendMeshLight));
			}
		}
	}
	break;
	...
	case MODEL_EFFECT_SAPITRES_ATTACK:
	{
		...
		{
			o->Position[0] += AdvanceByDelta((float)((rand() % 120) - 60));
			o->Position[1] += AdvanceByDelta((float)((rand() % 120) - 60));
	...
	case MODEL_LAVAGIANT_FOOTPRINT_V:
	{
		...
		o->Light[0] *= ReduceWithDelta(0.93f);
		o->Light[1] *= ReduceWithDelta(0.93f);
		o->Light[2] *= ReduceWithDelta(0.93f);
	}
	break;
	case MODEL_PROJECTILE:
	{
	...
	case MODEL_EFFECT_EG_GUARDIANDEFENDER_ATTACK2:
	{
		if (o->LifeTime <= 18)
		{
			o->Light[0] /= ReduceWithDelta(1.4f);
			o->Light[1] /= ReduceWithDelta(1.4f);
			o->Light[2] /= ReduceWithDelta(1.4f);
			o->Scale *= ReduceWithDelta(1.1f);
		}
	...
	case BITMAP_WATERFALL_4:
	{
		...
		o->Timer += AdvanceByDelta(0.1f);
		o->Distance += AdvanceByDelta(2.1f);
		o->Angle[0] += AdvanceByDelta(0.7f);
	...
	case MODEL_WOLF_HEAD_EFFECT:
	{
		...
		{
			o->Owner->Velocity += AdvanceByDelta(2.5f);
		}
		else if (o->SubType == 2)
		{
			o->Owner->Velocity += AdvanceByDelta(5.5f);
		}
	}
	break;
	case BITMAP_SBUMB:
	...
	case MODEL_SHOCKWAVE01:
	{
		...
		{
			o->Alpha = 1.0f;
			o->Scale *= ReduceWithDelta(1.24f);
			o->Position[2] *= ReduceWithDelta(1.19f);
			VectorScale(o->Light, ReduceWithDelta(0.45f), o->Light);
			if (o->Light[0] < 0.001f)
			{
		...
		{
			o->Scale *= ReduceWithDelta(1.15f);
			VectorScale(o->StartPosition, ReduceWithDelta(1.04f), o->StartPosition);
			VectorSubtract(o->Position, o->StartPosition, o->Position);
			VectorScale(o->Light, ReduceWithDelta(1.5f), o->Light);
			if (o->Light[0] > 0.6f)
			{
				o->SubType = 5;
			}
		...
		else
		{
			o->Alpha = 1.0f;
			o->Scale += AdvanceByDelta(0.2f);
			VectorScale(o->StartPosition, ReduceWithDelta(0.99f), o->StartPosition);
			VectorSubtract(o->Position, o->StartPosition, o->Position);
			VectorScale(o->Light, ReduceWithDelta(0.65f), o->Light);
	...
	case MODEL_SHOCKWAVE02:
	{
		...
		o->Scale += AdvanceByDelta(0.1f);
		VectorScale(o->StartPosition, ReduceWithDelta(0.95f), o->StartPosition);
		VectorSubtract(o->Position, o->StartPosition, o->Position);
	}
	break;
	case BITMAP_DAMAGE1:
	...
	case MODEL_SHOCKWAVE_SPIN01:
	{
		...
		if (o->SubType == 1)
		{
			o->Angle[1] += AdvanceByDelta(20.0f);
		}
	...
	case MODEL_WINDFOCE:
	{
		...
		else if (o->SubType == 1)		//�������ΰ� 
		{
			...
			VectorScale(o->Light, ReduceWithDelta(fFadeInOut), o->Light);
			o->Scale = 1.0f;
			VectorCopy(o->Owner->Position, o->Position);
		}
		else if (o->SubType == 2)
		{
			...
			o->Alpha *= ReduceWithDelta(0.92f);
			VectorScale(o->Light, ReduceWithDelta(0.85f), o->Light);
	...
	case MODEL_WINDFOCE_MIRROR:
	{
		...
		o->Scale *= ReduceWithDelta(1.015f);
	...
	case MODEL_SHOCKWAVE_GROUND01:
	{
		...
		{
			o->Scale *= ReduceWithDelta(1.2f);
			VectorScale(o->Light, ReduceWithDelta(0.7f), o->Light);
		}
		else
		{
			o->Scale *= ReduceWithDelta(1.2f);
	...
	case MODEL_WIND_SPIN03:
	{
		...
		if (o->LifeTime <= 30)
		{
			o->Scale += AdvanceByDelta(0.04);
			o->Angle[2] -= AdvanceByDelta(10.0);
			o->Position[2] = ow->Position[2] + (100.0);
			o->Light[0] *= ReduceWithDelta(0.85);
			o->Light[1] *= ReduceWithDelta(0.85);
			o->Light[2] *= ReduceWithDelta(0.85);
		...
		o->Angle[2] -= AdvanceByDelta(70.0);
		VectorScale(o->Light, ReduceWithDelta(0.95), o->Light);
	...
	case MODEL_WIND_SPIN01:
	{
		if (o->Owner != NULL && o->Owner->m_sTargetIndex >= 0)
		{
			...
			o->Scale += AdvanceByDelta(0.4f);
			o->Angle[2] += AdvanceByDelta(o->Scale * 10.f + 10.f);
			//o->Light[0] *= ReduceWithDelta(0.78f);
			//o->Light[1] *= ReduceWithDelta(0.78f);
			//o->Light[2] *= ReduceWithDelta(0.78f);
			VectorScale(o->Light, ReduceWithDelta(0.78f), o->Light);
			VectorCopy(Position, o->Position);
		}
		else
		{
		...
		if (o->Live)
		{
			if (o->Visible)
			{
				...
				case BITMAP_MAGIC + 1:
					...
					{
						VectorCopy(o->Owner->Position, o->Position);
						o->Angle[2] += AdvanceByDelta(10.0f);
					}
