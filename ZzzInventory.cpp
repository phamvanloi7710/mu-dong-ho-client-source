...
void RenderItemInfo(int sx, int sy, ITEM* ip, bool Sell, int Inventype, bool bItemTextListBoxUse, bool PostItem)
{
	if (ip->Type == -1)
		return;
	tm* ExpireTime;
	...
	if (!Sell && (ip->Type == ITEM_HELPER + 4 || ip->Type == ITEM_HELPER + 5)) // SPK Fix Tooltip Bird, Horse DL
	{
...
#if M_CUSTOM_ITEM
	gCustomItem.ItemInfo(ip->Type, TextList, TextListColor, TextBold, &TextNum);
#endif
	...
	if (ip->Type >= ITEM_POTION + 244 && ip->Type <= ITEM_POTION + 252)
	{
		sprintf(TextList[TextNum], GlobalText[4500]);
		TextListColor[TextNum] = TEXT_COLOR_YELLOW;
		TextBold[TextNum] = false;
		TextNum++;
	}
	else if (ip->Type >= ITEM_POTION + 23 && ip->Type <= ITEM_POTION + 26)
	{
	...
	*/
		...
		)
	{
		...
		else if ((bDurExist) && (ip->bPeriodItem == false) && ip->Type != ITEM_HELPER + 121)
		{
			int maxDurability = calcMaxDurability(ip, p, Level);
	...
	if (ip->RequireStrength && bRequireStat)
	{
		...
		DWORD Strength;
		Strength = CharacterAttribute->AddStrength;
		auto need = (ip->RequireStrength - si_iNeedStrength);
		if (static_cast<unsigned int>(Strength) < static_cast<unsigned int>(need))
		{
			TextListColor[TextNum] = TEXT_COLOR_RED;
			TextBold[TextNum] = false;
			TextNum++;
	...
	if (ip->RequireDexterity && bRequireStat)
	{
		...
		DWORD Dexterity;
		Dexterity = CharacterAttribute->AddDexterity;
		auto need = (ip->RequireDexterity - si_iNeedDex);
		if (static_cast<unsigned int>(Dexterity) < static_cast<unsigned int>(need))
		{
			TextListColor[TextNum] = TEXT_COLOR_RED;
			TextBold[TextNum] = false;
			TextNum++;
	...
	if (ip->RequireVitality && bRequireStat)
	{
		...
		DWORD Vitality;
		Vitality = CharacterAttribute->AddVitality;
	...
	if (ip->RequireEnergy && bRequireStat)
	{
		...
		DWORD Energy;
		Energy = CharacterAttribute->AddEnergy;
	...
	if (ip->RequireCharisma && bRequireStat)
	{
		...
		DWORD Charisma;
		Charisma = CharacterAttribute->AddCharisma;
...
#if defined(LJH_ADD_ITEMS_EQUIPPED_FROM_INVENTORY_SYSTEM) || \
    defined(LJH_ADD_ITEMS_EQUIPPED_FROM_INVENTORY_SYSTEM_PART_2)
	...
	if (!bItemTextListBoxUse)
	{
		...
		if (fRateY != 0.0f)
		{
			Height = (TextLine * TextSize.cy +
				EmptyLine * TextSize.cy / 2.0f) / fRateY;
		}
		else
		{
			Height = 0.0f; // hoặc giá trị fallback hợp lý
		}
	...
	int ItemIndex = ip->Type;
	TextNum = GInfo.nInformationOP(TextNum, ip);
	...
	if (isrendertooltip)
	{
		...
		{
			RenderTipTextList(sx, sy, TextNum, 0, RT3_SORT_CENTER, 0, true, true);
		}
		else
		{
...
void RenderObjectScreen(int Type, int ItemLevel, int Option1, int ExtOption, vec3_t Target, int Select, bool PickUp, float Scale)
{
	...
	if (Select == TRUE)
	{
		...
		ObjectSelect.Angle[1] = lasted_rotation + AdvanceByDelta(14);
