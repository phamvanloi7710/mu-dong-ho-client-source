...
void CreateFrustrum2D(vec3_t Position, bool is_wide)
{
	float Width = 0.0f;
	float WidthFar = 0.0f;
	float WidthNear = 0.0f;
	float CamViewFar = 0.0f;
	float CameraViewNear = 0.0f;
	float CameraViewTarget = 0.0f;

	if ((gmProtect->ScreenType == 0 && !gmProtect->checknew_school()) || (gmProtect->ScreenType != 0 && (WindowWidth < 1024 && !gmProtect->checknew_school())))
	{
		Width = (float)GetScreenWidth() / (float)(GetWindowsY - 48.0f);
	}
	else
	{
		Width = (float)WindowWidth / (float)WindowHeight;
	}

	if (World == WD_6STADIUM && (FindText(Hero->ID, "webzen") || FindText(Hero->ID, "webzen2")))
	{
		CamViewFar = 8500.0f;
		CameraViewNear = CamViewFar * 0.05f;
		CameraViewTarget = CamViewFar * 0.47f;
	...
	else
	{
		if (gMapManager->InBattleCastle())
		{
			if (battleCastle::InBattleCastle2(Hero->Object.Position) && (Hero->Object.Position[0] < 17100.f || Hero->Object.Position[0]> 18300.f))
			{
				CamViewFar = 5100.0f;// * 0.1f;
				CameraViewNear = CamViewFar * 0.19f;//0.22
				CameraViewTarget = CamViewFar * 0.47f;//0.47
				WidthFar = 2250.0f * Width; // 1140.f
				WidthNear = 540.0f * Width; // 540.f
			}
			else
			{
				CamViewFar = 3300.0f;// * 0.1f;
				CameraViewNear = CamViewFar * 0.19f;//0.22
				CameraViewTarget = CamViewFar * 0.47f;//0.47
				WidthFar = 1300.0f * Width; // 1140.f
				WidthNear = 580.0f * Width; // 540.f
			}
		}
		else if (World == WD_62SANTA_TOWN)
		{
			CamViewFar = 2850.0;
			CameraViewNear = CamViewFar * 0.19f;
			CameraViewTarget = CamViewFar * 0.47f;
		...
		{
			CamViewFar = 3300.0f;
			CameraViewNear = CamViewFar * 0.0166666666666667;
			CameraViewTarget = CamViewFar * 0.2515151515151515;
			WidthFar = Width * 1900.0f;
			WidthNear = Width * 600.0f;
		}
		else
		{
			//static int CameraLevel;
			...
			else
			{
				...
				if (SceneFlag == LOG_IN_SCENE)
				{
					...
					else
					{
						if (gmProtect->SceneLogin != 0)
						{
							CamViewFar = gCameraManager->CalcViewFar(2400.0);
							Width = gCameraManager->CalcWidthCam();
				...
				else
				{
					...
					if (conf)
					{
						if (World == WD_39KANTURU_3RD)
							CamViewFar = conf->view_far * 0.8f;
						else
							CamViewFar = conf->view_far;

						CameraViewNear = CamViewFar * 0.19f;
						CameraViewTarget = CamViewFar * 0.47f;
						WidthFar = conf->width_far * Width;
						WidthNear = conf->width_near * Width;
					}
					else
					{
						if (World == WD_39KANTURU_3RD)
	...
	Vector(-WidthFar, CamViewFar - CameraViewTarget, 0.f, p[0]);
	Vector(WidthFar, CamViewFar - CameraViewTarget, 0.f, p[1]);
	Vector(WidthNear, CameraViewNear - CameraViewTarget, 0.f, p[2]);
	Vector(-WidthNear, CameraViewNear - CameraViewTarget, 0.f, p[3]);
...
void CreateFrustrum(float Aspect, vec3_t position, bool wide)
{
	float Distance = CameraViewFar * 0.9;
	float radiansFOV = tanf((CameraFOV * Q_PI / 180.f) * 0.5f);
	float Width = ((radiansFOV * Distance) * Aspect) + 100.0;
	float Height = Width * 3.f / 4.f;
	...
	CreateFrustrum2D(position, wide);
}
