...
bool PetActionCollecterAdd::Move(OBJECT* obj, CHARACTER* Owner, int targetKey, double tick, bool bForceRender)
{
	...
	case eAction_Stand:
		{
			...
			if (fastspeedter)
				obj->Direction[1] = -AdvanceByDelta(Speed);
			else
	...
	case eAction_Return:
	{
		...
		obj->Position[0] += AdvanceByDelta(Direction[0]);
		obj->Position[1] += AdvanceByDelta(Direction[1]);
		obj->Position[2] += AdvanceByDelta(Direction[2]);
...
void PetActionCollecterAdd::FindZen(OBJECT* obj)
{
   ...
   for(int i=0;i<MAX_ITEMS;i++)   
   {
      ...
      if( SEARCH_LENGTH > dl )
      {
         if (Items[i].Item.Type == ITEM_POTION + 15 || isJewels(Items[i].Item.Type) == 1)
         {
            m_isRooting = true;
...
bool PetActionCollecterSkeleton::Move(OBJECT* obj, CHARACTER* Owner, int targetKey, double tick, bool bForceRender)
{
	...
	case eAction_Get:
	{
		...
		float Angle = CreateAngle(obj->Position[0], obj->Position[1], targetPos[0], targetPos[1]);
		obj->Angle[2] = TurnAngle2(obj->Angle[2], Angle, AdvanceByDelta(10.0f));
