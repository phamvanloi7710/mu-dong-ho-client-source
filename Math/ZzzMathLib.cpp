...
void interpolatesum(vec3_t v_out, vec3_t b)
{
	if (v_out && b)
	{
		v_out[0] += AdvanceByDelta(b[0]);
		v_out[1] += AdvanceByDelta(b[1]);
		v_out[2] += AdvanceByDelta(b[2]);
	}
}

void interpolatesum2(vec3_t a, vec3_t b, vec3_t v_out)
{
	v_out[0] = a[0] + AdvanceByDelta(b[0]);
	v_out[1] = a[1] + AdvanceByDelta(b[1]);
	v_out[2] = a[2] + AdvanceByDelta(b[2]);
}
