...
WORD g_ServerPort = 70000;
char* szServerIpAddress = "192.168.1.140";
...
void CreateLogInScene()
{
...
#if USING_RECONNECT_ACCOUNT
	//g_PLReconnect->ReconnectCreateConnection(szServerIpAddress, g_ServerPort);
	...
	if (g_iChatInputType == 0)
	{
		...
		if (InputLength[0] == 0)
			...
			InputIndex = 1;*/
		strcpy(InputText[0], m_Current_Acc);
		InputLength[0] = strlen(InputText[0]);
		InputTextMax[0] = MAX_ID_SIZE;
...
bool MoveMainCamera()
{
	...
	else
	{
		...
		if ((TerrainWall[iIndex] & TW_CAMERA_UP) == TW_CAMERA_UP)
		{
			...
			{
				g_fCameraCustomDistance += AdvanceByDelta(10);
			}
		}
		else
		{
...
void MoveMainScene()
{
	...
	EarthQuake *= ReduceWithDelta(0.2f);
...
float colorworld[4] = { 0.0, 0.0 , 0.0 , 1.f };
bool RenderMainScene()
{
	if (!EnableMainRender || (LoadingWorld) > 30)
	{
		return false;
	}
	...
	{
		colorworld[0] = 10.f / 256.f;
		colorworld[1] = 20.f / 256.f;
		colorworld[2] = 14.f / 256.f;
	}
	else if (World == WD_2DEVIAS)
	{
		colorworld[0] = 0.0f / 256.f;
		colorworld[1] = 0.0f / 256.f;
		colorworld[2] = 10.f / 256.f;
	}
	else if (World == WD_10HEAVEN)
	{
		colorworld[0] = 3.0f / 256.f;
		colorworld[1] = 25.0f / 256.f;
		colorworld[2] = 44.0f / 256.f;
	}
	else if (World >= WD_45CURSEDTEMPLE_LV1 && World <= WD_45CURSEDTEMPLE_LV6)
	{
		colorworld[0] = 9.f / 256.f;
		colorworld[1] = 8.f / 256.f;
		colorworld[2] = 33.f / 256.f;
	}
	else if (gMapManager->InChaosCastle() || gMapManager->InHellas())
	{
		byWaterMap = gMapManager->InHellas();
		colorworld[0] = 30.f / 256.f;
		colorworld[1] = 40.f / 256.f;
		colorworld[2] = 40.f / 256.f;
	}
	else
	{
		colorworld[0] = 20.f / 256.f;
		colorworld[1] = 20.f / 256.f;
		colorworld[2] = 20.f / 256.f;
	}

	FogEnable = CameraFOV > 35.0f;

	glClearColor(colorworld[0], colorworld[1], colorworld[2], 1.f);

	if (CameraTopViewEnable == false)
	{
#if MAIN_UPDATE > 303 && MAIN_UPDATE < 903
		if ((gmProtect->ScreenType == 0 && !gmProtect->checknew_school()) || (gmProtect->ScreenType != 0 && (WindowWidth < 1024 && !gmProtect->checknew_school())))
		{
			Width = GetScreenWidth();
			Height = gwinhandle->GetScreenY() - 48.0f;
		}
		else
		{
			Width = gwinhandle->GetScreenX();
			Height = gwinhandle->GetScreenY();
		}
#else
		Width = gwinhandle->GetScreenX();
		Height = gwinhandle->GetScreenY();
	...
	//gCameraController->UpdateSmoothCamera();

	BeginOpengl(0, 0, Width, Height, true);
	//CreateFrustrum((float)Width / (float)GetScreenWidth(), pos);
	CreateFrustrum((float)Width / (float)gwinhandle->GetScreenX(), pos, g_pInvenExpansion->GetThirdInterface());
...
void MainScene(HDC hDC)
{
	...
	const int32_t fixedDeltaTime = 40;
	static int32_t accumulatedTime = fixedDeltaTime;
...
#ifdef PROCESS_FPS_NEW
	gsteady_clock->setTradicional(true);

	for (Remain = accumulatedTime; Remain >= fixedDeltaTime; Remain -= fixedDeltaTime)
#endif // PROCESS_FPS_NEW
	{
#ifndef PROCESS_FPS_NEW
...
#ifdef PROCESS_FPS_NEW
	gsteady_clock->setTradicional(false);
#endif // PROCESS_FPS_NEW
	if (Destroy)
	{
		return;
	...
	if (EnableSocket && SceneFlag == MAIN_SCENE)
	{
		#if USING_RECONNECT_ACCOUNT
			// New
			if (SocketClient.GetSocket() == INVALID_SOCKET && g_PLReconnect->s_Data.ReconnectStatus != g_PLReconnect->RECONNECT_STATUS_RECONNECT)
			{
