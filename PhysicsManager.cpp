...
void CPhysicsVertex::UpdateForce(unsigned int iKey, DWORD dwType, float fWind)
{
	...
	{
	case PCT_RUBBER:
		m_vForce[2] += (fRand * (fWind + 0.1f) * 1.f) * AdvanceByDelta();
		break;
	case PCT_RUBBER2:
		m_vForce[2] += (fRand * fWind) * AdvanceByDelta();
		break;
	}
	...
	case PCT_ELASTIC_HALLOWEEN:
		m_vForce[0] += -(fRand * fWind * 0.5f) * AdvanceByDelta();
		m_vForce[2] += (fRand * (fWind + 0.1f) * 0.5f * (float)sinf(WorldTime * 0.003f) * 5.0f) * AdvanceByDelta();
		m_vForce[2] -= (s_Gravity * fGravityRate * s_fMass * 50.0f) * AdvanceByDelta();
		break;
	case PCT_ELASTIC_RAGE_L:
		m_vForce[0] += -((fRand * fWind * 0.8f) * AdvanceByDelta());
		break;
	...
	{
	case PCT_HEAVY:
		m_vForce[2] -= (s_Gravity * fGravityRate * s_fMass * 180.0f) * AdvanceByDelta();
		break;
	default:
		m_vForce[2] -= (s_Gravity * fGravityRate * s_fMass * 100.0f) * AdvanceByDelta();
		break;
	}
}
...
void CPhysicsCloth::InitForces()
{
	...
	{
		m_pVertices[iVertex].UpdateForce(abs(iSeed % m_iNumHor - iVertex % m_iNumHor) + abs(iSeed / m_iNumHor - iVertex / m_iNumHor), m_dwType, (m_fWind) * AdvanceByDelta(1.0));
	}
}
...
void CPhysicsCloth::MoveVertices(float fTime)
{
		...
		if (pLink->m_byStyle & PLS_SPRING)
		{
			...
			if (fDistance > pLink->m_fDistance[1] + 0.01f)
			{
				...
				for (int i = 0; i < 3; ++i)
				{
					...
					{
						vForce[i] *= ReduceWithDelta((pLink->m_fDistance[1] / 32.0f));
					}
					switch (PCT_MASK_ELASTIC & m_dwType)
					{
					case PCT_RUBBER:
						vForce[i] *= ReduceWithDelta(3.0f);
						break;
					}
					...
					{
					case PCT_ELASTIC_HALLOWEEN:
						vForce[i] *= ReduceWithDelta(2.0f);
						break;
					case PCT_ELASTIC_RAGE_L:
					case PCT_ELASTIC_RAGE_R:
					{
						vForce[i] *= ReduceWithDelta(1.0f);
					}
					break;
					}
				}
	...
	for (int iVertex = 0; iVertex < m_iNumVertices; ++iVertex)
	{
		m_pVertices[iVertex].Move(AdvanceByDelta(fTime));
	}
}
...
void CPhysicsManager::Move(float fTime)
{
	...
	CPhysicsManager::s_fWind += (fPlus) * AdvanceByDelta(1.0);
