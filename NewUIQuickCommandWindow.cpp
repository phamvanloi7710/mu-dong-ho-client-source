...
bool SEASON3B::CNewUIQuickCommandWindow::UpdateMouseEvent()
{
	...
	for (int i = 0; i < eEndCommand; ++i) 
	{
		if (CheckMouseIn(pt.x, pt.y, eWidth, 19) == true) 
		{
			m_iSelectedIndex = i;
			break;
		}
	...
	if (m_iSelectedIndex > -1 && SEASON3B::IsRelease(VK_LBUTTON))
	{
		...
		case eViewInfo:
		{
			SendRequestViewCharItem(pCha->Key);
			CloseQuickCommand();
			return false;
		}
		break;
	...
	if (CheckMouseIn(m_Pos.x, m_Pos.y + 30, eWidth, eHeight) == false)
	{
		m_iSelectedIndex = -1;
	...
	if (CheckMouseIn(m_Pos.x, m_Pos.y, eWidth, eHeight))
	{
		return false;
	}
...
void SEASON3B::CNewUIQuickCommandWindow::RenderFrame()
{
	...
	x = m_Pos.x; y = m_Pos.y; width = eWidth; height = eHeight; 
...
void SEASON3B::CNewUIQuickCommandWindow::RenderContents()
{
	...
	char* title[eEndCommand] = { "Giao Dịch", "Xem Cửa Hàng", "Tạo Nhóm", "Đi Theo", "Quyết Đáu","Xem Thông Tin" };
	...
	for (int i = 0; i < eEndCommand; ++i)
	{
		...
		g_pRenderText->RenderText(m_Pos.x, y, title[i], 112, 0, RT3_SORT_CENTER);
		y += 19.f;
	}
}
