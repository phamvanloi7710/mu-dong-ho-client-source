	...
	if (o->Live)
	{
		...
		case Movement::Muun2Pet:
		{
			...
			if (Distance >= MoveRange * MoveRange)
			{
				...
				{
					o->Position[0] += (float)AdvanceByDelta(MoveX);
					o->Position[1] += (float)AdvanceByDelta(MoveY);
					o->Angle[2] = 180;
					o->Angle[2] -= (Angle2);
				}
			...
			if (Distance >= LootPosition * LootPosition)
			{
				if (o->Position[0] < o->Owner->Position[0])
				{
					...
					o->Position[0] += AdvanceByDelta(radial);
				}
				...
				else if (o->Position[1] < o->Owner->Position[1])
				{
					...
					o->Position[1] += AdvanceByDelta(radial);
				}
				else if (o->Position[1] > o->Owner->Position[1])
				{
					float radial = Range[1] - rand() % 360;
			...
			{
				o->Direction[2] += AdvanceByDelta(1.5f);
			}
		...
		if (o->movementType == Movement::Angel || o->Type == MODEL_HELPER + 1)
		{
			...
			o->Position[2] += AdvanceByDelta((float)(rand() % 16 - 8));
...
void RenderDarkHorseSkill(OBJECT* pObject, BMD* pModel)
{
	...
	{
		pObject->WeaponLevel++;
		pObject->LifeTime += AdvanceByDelta(1.f);
...
void MoveBat(OBJECT* o)
{
	...
	o->Position[2] += AdvanceByDelta(-absf(sinf(o->Timer)) * 150.f + 350.f);
	o->Timer += AdvanceByDelta(0.2f);
}
...
void MoveBird(OBJECT* o)
{
	...
	{
		o->Position[2] += AdvanceByDelta((float)(rand() % 16 - 8));
		o->Velocity -= AdvanceByDelta(0.005f);
		if (o->Velocity <= 1.f)
		{
			o->AI = BOID_FLY;
		}
...
void MoveHeavenBug(OBJECT* o, int index)
{
	...
	o->Position[0] += AdvanceByDelta(o->Velocity * (float)sinf(o->Angle[2]));
	o->Position[1] -= AdvanceByDelta(o->Velocity * (float)cosf(o->Angle[2]));
	double angle = 0.01f * cosf((float)(34571 + iFrame + index * 41273) * 0.0003f) * sinf((float)(17732 + iFrame + index * 5161) * 0.0003f);
	o->Angle[2] += AdvanceByDelta(angle);
...
void MoveEagle(OBJECT* o)
{
	...
	o->Position[2] += AdvanceByDelta(sinf(WorldTime * 0.0005f) * 1.0f);
	o->Angle[1] += AdvanceByDelta(sinf(WorldTime * 0.001f) * 0.4f);
...
void MoveTornado(OBJECT* o)
{
	...
	o->Position[0] += AdvanceByDelta(sinf(o->HeadAngle[0]) * 2.0f);
	o->Position[1] += AdvanceByDelta(cosf(o->HeadAngle[0]) * 2.0f);
	o->Angle[2] = 0;
		...
		if (o->Live)
		{
			...
			if (pModel)
			{
				...
				if (EnableEvent != 0 && o->Type == MODEL_MONSTER01 + 31)
				{
					...
					o->Timer += AdvanceByDelta(o->Scale * 0.05f);
