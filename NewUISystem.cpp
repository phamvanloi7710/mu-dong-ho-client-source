...
SEASON3B::CNewUISystem::CNewUISystem()
{
	...
	m_pNewUIMixExpansion = NULL;
		...
		m_NewUIPurchaseMoney = NULL;
		m_NewUIPurchaseMoneyDetail = NULL;
		m_NewUIPurchaseMoneyCancelPayment = NULL;
	#endif // USING_RECHARGE_MONEY

	#if USING_RESET_WITH_GUI
		m_NewUIResetSystem = NULL;
	#endif // USING_RESET_WITH_GUI
	m_NewUINotification = NULL;
}
...
bool SEASON3B::CNewUISystem::Create()
{
	...
	// -- Phong: khởi tạo UI Mốc Nạp
	m_pNewUIMocNap = new CNewUIMocNap;
	if (m_pNewUIMocNap->Create(m_pNewUIMng, 50, 50) == false)
	{
		return false;
	}
...
bool SEASON3B::CNewUISystem::LoadMainSceneInterface()
{
	...
	m_pThangCustomDraw = new CNewUIPhongDraw;
	if (m_pThangCustomDraw->Create(m_pNewUIMng, 0, 0) == false)
	{
		return false;
	}
	...
	m_pPCCustomDraw = new CNewUIPCDraw;
	if (m_pPCCustomDraw->Create(m_pNewUIMng, 0, 0) == false)
	{
		return false;
	}

	m_NewUIRankTop = new CNewUIRankingTop;
	int eWidthRanking = CNewUIRankingTop::WindowsSize::eMainWidth;
	int eHeightRanking = CNewUIRankingTop::WindowsSize::eMainHeight;
	if (m_NewUIRankTop->Create(m_pNewUIMng, PositionX_The_Mid(eWidthRanking), PositionY_The_Mid(eHeightRanking)) == false)
	{
		return false;
	}
	...
	if (m_NewUIMenuUser->Create(m_pNewUIMng, AddMiddleX(230), 100) == false)
	{
		return false;
	}
	...
	m_NewUIViewCharInfo = new CNewUIViewCharInfo;
	int eWidthViewCharInfo = CNewUIViewCharInfo::WindowsSize::eMainWidth;
	int eHeightViewCharInfo = CNewUIViewCharInfo::WindowsSize::eMainHeight;
	if (m_NewUIViewCharInfo->Create(m_pNewUIMng, m_pNewUI3DRenderMng, PositionX_The_Mid(eWidthViewCharInfo), PositionY_The_Mid(eHeightViewCharInfo)) == false)
	{
		return false;
	}

	#if USING_RECHARGE_MONEY
		m_NewUIPurchaseMoney = new PLCRechargeMoney;

		if (m_NewUIPurchaseMoney->Create(m_pNewUIMng, AddMiddleX(230), 100) == false)
		{
			return false;
		}
		m_NewUIPurchaseMoneyDetail = new PLCRechargeMoneyDetail;

		if (m_NewUIPurchaseMoneyDetail->Create(m_pNewUIMng, AddMiddleX(230), 100) == false)
		{
			return false;
		}
		m_NewUIPurchaseMoneyCancelPayment = new PLCRechargeMoneyPaymentCancel;

		if (m_NewUIPurchaseMoneyCancelPayment->Create(m_pNewUIMng, AddMiddleX(230), 100) == false)
		{
			return false;
		}
	#endif // USING_RECHARGE_MONEY
	...
	#if USING_RESET_WITH_GUI
		m_NewUIResetSystem = new CPLResetSystem;
		...
		m_NewUINotification = new PLCNotification;

	if (m_NewUINotification->Create(m_pNewUIMng, AddMiddleX(230), 100) == false)
	{
		return false;
	}
	m_iCamWebzen->Init();
...
void SEASON3B::CNewUISystem::UnloadMainSceneInterface()
{
	...
	SAFE_DELETE(m_NewUIViewCharInfo); 

	#if USING_RECHARGE_MONEY
		SAFE_DELETE(m_NewUIPurchaseMoney);
		SAFE_DELETE(m_NewUIPurchaseMoneyDetail);
		SAFE_DELETE(m_NewUIPurchaseMoneyCancelPayment);
	#endif // USING_RECHARGE_MONEY

	#if USING_RESET_WITH_GUI
		SAFE_DELETE(m_NewUIResetSystem);
	#endif // USING_RESET_WITH_GUI

	SAFE_DELETE(m_NewUINotification);
...
void SEASON3B::CNewUISystem::Show(DWORD dwKey)
{
	...
	if (m_pNewUIMng)
	{
		...
		case SEASON3B::INTERFACE_VIEW_CHARINFO:
			g_pViewCharInfo->OpenningProcess();
			HideGroupOpenInterface(INTERFACE_VIEW_CHARINFO);
			break;
		...
		case SEASON3B::INTERFACE_SHOW_VIP:
			...
			break;
		#if USING_RECHARGE_MONEY
			case SEASON3B::INTERFACE_RECHARGE_MONEY_SYSTEM:
				m_NewUIPurchaseMoney->OpenningProcess();
				break;
			case SEASON3B::INTERFACE_RECHARGE_MONEY_DETAIL_MAIN:
				m_NewUIPurchaseMoneyDetail->OpenningProcess();
				break;
			case SEASON3B::INTERFACE_RECHARGE_MONEY_CANCEL_PAYMENT_MAIN:
				m_NewUIPurchaseMoneyCancelPayment->OpenningProcess();
				break;
		#endif // USING_RECHARGE_MONEY
		#if USING_RESET_WITH_GUI
			case SEASON3B::INTERFACE_RESET_SYSTEM:
				m_NewUIResetSystem->OpenningProcess();
				break;
		#endif // USING_RESET_WITH_GUI
		case SEASON3B::INTERFACE_NOTIFICATION_MAIN:
			m_NewUINotification->OpenningProcess();
			break;
			}
		...
		default:
			if (dwKey != SEASON3B::INTERFACE_MYSHOP_INVENTORY && dwKey != SEASON3B::INTERFACE_PURCHASESHOP_INVENTORY)
			{
				...
				case SEASON3B::INTERFACE_CATAPULT:
					...
					break;
				case SEASON3B::INTERFACE_VIEW_CHARINFO:
					g_pViewCharInfo->ClosingProcess();
					break;
				case SEASON3B::INTERFACE_CHAOSCASTLE_TIME:
				...
				default:
					if (dwKey != SEASON3B::INTERFACE_OPTION)
					{
						...
						#if USING_RECHARGE_MONEY
							case SEASON3B::INTERFACE_RECHARGE_MONEY_SYSTEM:
								m_NewUIPurchaseMoney->ClosingProcess();
								break;
							case SEASON3B::INTERFACE_RECHARGE_MONEY_DETAIL_MAIN:
								m_NewUIPurchaseMoneyDetail->ClosingProcess();
								break;
							case SEASON3B::INTERFACE_RECHARGE_MONEY_CANCEL_PAYMENT_MAIN:
								m_NewUIPurchaseMoneyCancelPayment->ClosingProcess();
								break;
						#endif // USING_RECHARGE_MONEY
						#if USING_RECHARGE_MONEY
							case SEASON3B::INTERFACE_RESET_SYSTEM:
								m_NewUIResetSystem->ClosingProcess();
								break;
						#endif // USING_RECHARGE_MONEY
						case SEASON3B::INTERFACE_NOTIFICATION_MAIN:
							m_NewUINotification->ClosingProcess();
							break;
						}
					}
					else
...
void SEASON3B::CNewUISystem::HideGroupOpenInterface(DWORD dwKey)
{
	DWORD dwGroupC[] =
	{
		...
		SEASON3B::INTERFACE_VIEW_CHARINFO,
...
void SEASON3B::CNewUISystem::RenderItem3DFree(float sx, float sy, float Width, float Height, int Type, int Level, int Option1, int ExtOption, bool PickUp, float Scale, bool FixY)
{
	...
	Script_Item* p = GMItemMng->find(Type);

	float BWidth = (float)p->Getwidth() * INVENTORY_SCALE;
	float BHeight = (float)p->Getheight() * INVENTORY_SCALE;
...
void SEASON3B::CNewUISystem::UpdateSendMoveInterface()
{
	...
	#if USING_RECHARGE_MONEY
		if (IsVisible(SEASON3B::INTERFACE_RECHARGE_MONEY_SYSTEM))
		{
			Hide(SEASON3B::INTERFACE_RECHARGE_MONEY_SYSTEM);
		}
		if (IsVisible(SEASON3B::INTERFACE_RECHARGE_MONEY_DETAIL_MAIN))
		{
			Hide(SEASON3B::INTERFACE_RECHARGE_MONEY_DETAIL_MAIN);
		}
		if (IsVisible(SEASON3B::INTERFACE_RECHARGE_MONEY_CANCEL_PAYMENT_MAIN))
		{
			Hide(SEASON3B::INTERFACE_RECHARGE_MONEY_CANCEL_PAYMENT_MAIN);
		}
	#endif // USING_RECHARGE_MONEY

	#if USING_RESET_WITH_GUI
		if (IsVisible(SEASON3B::INTERFACE_RESET_SYSTEM))
		{
			Hide(SEASON3B::INTERFACE_RESET_SYSTEM);
		}
	#endif // USING_RESET_WITH_GUI

	if (IsVisible(SEASON3B::INTERFACE_NOTIFICATION_MAIN))
	{
		//SendRequestTradeExit();
		Hide(SEASON3B::INTERFACE_NOTIFICATION_MAIN);
	}
}
...
CNewUIPhongDraw* SEASON3B::CNewUISystem::GetUI_NewThangCustomDraw() const
{
	return m_pThangCustomDraw;
}

CNewUIViewCharInfo* SEASON3B::CNewUISystem::GetUI_NewUIViewCharInfo() const
{
	return m_NewUIViewCharInfo;
}


CNewUIPCDraw* SEASON3B::CNewUISystem::GetUI_NewPCCustomDraw() const
{
	return m_pPCCustomDraw;
}
...
#if USING_RECHARGE_MONEY
	PLCRechargeMoney* SEASON3B::CNewUISystem::GetUI_PLCRechargeMoney() const
	{
		return m_NewUIPurchaseMoney;
	}
	PLCRechargeMoneyDetail* SEASON3B::CNewUISystem::GetUI_PLCRechargeMoneyDetail() const
	{
		return m_NewUIPurchaseMoneyDetail;
	}
	PLCRechargeMoneyPaymentCancel* SEASON3B::CNewUISystem::GetUI_PLCRechargeMoneyPaymentCancel() const
	{
		return m_NewUIPurchaseMoneyCancelPayment;
	}
#endif // USING_RECHARGE_MONEY

#if USING_RESET_WITH_GUI
	CPLResetSystem* SEASON3B::CNewUISystem::GetUI_CPLResetSystem() const
	{
		return m_NewUIResetSystem;
	}
#endif // USING_RESET_WITH_GUI

PLCNotification* SEASON3B::CNewUISystem::GetUI_PLCNotification() const
{
	return m_NewUINotification;
}
