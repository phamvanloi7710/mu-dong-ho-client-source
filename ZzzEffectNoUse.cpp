...
void RenderSky3(float* color)
{
	...
	float distance = CameraViewFar * 1.3;
	...
	for (float x = 0.f; x < Num; x += 1.f)
	{
		...
		for (int i = 0; i < 4; i++)
		{
			glColor4f(Light[i][0], Light[i][1], Light[i][2], 0.8);
			//glTexCoord2f(UV[i][0], UV[i][1]);
			glVertex3f(Position[i][0], Position[i][1], Position[i][2] - 35.f);
		}
		glEnd();
	}
}
