...
const char* CCharacterManager::GetGuildStatusText(const int GuildStatus)
{
	switch (GuildStatus)
	{
	case G_PERSON:
		return GlobalText[1330];
	case G_MASTER:
		return GlobalText[1300];
	case G_SUB_MASTER:
		return GlobalText[1301];
	case G_BATTLE_MASTER:
		return GlobalText[1302];
	default:
		return "-----";
		break;
	}
