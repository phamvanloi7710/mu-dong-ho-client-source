...
void CLog::AddLog(BOOL active, const char* directory) // OK
{
	...
	if (lpInfo->Active != 0)
	{
		CreateDirectoryA(lpInfo->Directory, 0);
		...
		sprintf(lpInfo->Filename, ".\\%s\\%04d-%02d-%02d.txt", lpInfo->Directory, lpInfo->Year, lpInfo->Month, lpInfo->Day);

		lpInfo->File = CreateFileA(lpInfo->Filename, GENERIC_WRITE, FILE_SHARE_READ, 0, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, 0);
