...
void ProjectionUI(vec3_t Position, int* sx, int* sy)
{
	vec3_t TransformPosition;
	VectorTransform(Position, CameraMatrix, TransformPosition);

	if (TransformPosition[2] < 1.0f)
		TransformPosition[2] = 1.0f;

	// Công thức chuẩn widescreen
	float px = TransformPosition[0] / TransformPosition[2];
	float py = TransformPosition[1] / TransformPosition[2];

	*sx = (int)(GetWindowsX / 2 + px * (GetWindowsX / (2.0f * tanf(CameraFOV * 0.5f * 3.14159f / 180.0f))));
	*sy = (int)(GetWindowsY / 2 - py * (GetWindowsY / (2.0f * tanf(CameraFOV * 0.5f * 3.14159f / 180.0f))));
}
...
void BeginOpenglUI(int x, int y, int Width, int Height)
{
	glMatrixMode(GL_PROJECTION);
	glPushMatrix();
	glLoadIdentity();

	// Ortho 2D mode theo độ phân giải hiện tại
...
void RenderBitmap(int Texture, float x, float y, float Width, float Height, float u, float v, float uWidth, float vHeight, bool Scale, bool StartScale, float Alpha)
{
	...
	// Habilitar el uso de punteros de vxrtices y coordenadas de textura
	glEnableClientState(GL_VERTEX_ARRAY);
	glEnableClientState(GL_TEXTURE_COORD_ARRAY);

	// Pasar los datos de los vxrtices y las coordenadas de textura a OpenGL
	glVertexPointer(2, GL_FLOAT, 0, p);  // 2 componentes por vxrtice (x, y)
	glTexCoordPointer(2, GL_FLOAT, 0, c);  // 2 componentes por coordenada de textura (u, v)
	...
	// Dibujar los vxrtices como un cuadrado utilizando un trixngulo en abanico
	glDrawArrays(GL_TRIANGLE_FAN, 0, 4);  // 4 vxrtices en total
