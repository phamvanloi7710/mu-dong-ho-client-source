...
{
	float AngleY = 0.0;
	o->Gravity += AdvanceByDelta(2.5f);
	o->Angle[0] -= AdvanceByDelta(4.f);
	o->m_bActionStart = true;
	o->Direction[1] += AdvanceByDelta(o->Direction[0]);
	...
	if (o->Type == MODEL_PLAYER)
	{
		for (int i = PLAYER_WALK_MALE; i <= PLAYER_WALK_CROSSBOW; i++)
		{
			...
			if (g_isCharacterBuff(o, eDeBuff_Freeze))
				pAction->PlaySpeed *= ReduceWithDelta(0.5f);
			else if (g_isCharacterBuff(o, eDeBuff_BlowOfDestruction))
				pAction->PlaySpeed *= ReduceWithDelta(0.33f);
		}
		...
		if (g_isCharacterBuff(o, eDeBuff_Freeze))
		{
			gmClientModels->GetPlayerModelAction(PLAYER_RUN_RIDE_HORSE)->PlaySpeed *= ReduceWithDelta(0.5f);
			gmClientModels->GetPlayerModelAction(PLAYER_DARKLORD_WALK)->PlaySpeed *= ReduceWithDelta(0.5f);
		}
		else if (g_isCharacterBuff(o, eDeBuff_BlowOfDestruction))
		{
			gmClientModels->GetPlayerModelAction(PLAYER_RUN_RIDE_HORSE)->PlaySpeed *= ReduceWithDelta(0.33f);
			gmClientModels->GetPlayerModelAction(PLAYER_DARKLORD_WALK)->PlaySpeed *= ReduceWithDelta(0.33f);
		}
...
void SetAttackSpeed()
{
	//float AttackSpeed1 = CharacterAttribute->AttackSpeed * 0.004f; //* 0.001f;   //aLongfix5 tra lai toc do goc)
	//float MagicSpeed1 = CharacterAttribute->MagicSpeed * 0.004f; //* 0.001f;   //aLongfix5 tra lai toc do goc)
	//float MagicSpeed2 = CharacterAttribute->MagicSpeed * 0.002f; //* 0.001f;   //aLongfix5 tra lai toc do goc)
...
//	}
//}
// LoiPham Fix: nhánh Dark Horse vào SetPlayerMagic để kích hoạt hoạt ảnh niệm phép khi cưỡi ngựa.
void SetPlayerMagic(CHARACTER* c)
{
	...
	if (o->Type == MODEL_PLAYER)
	{
		...
		else if ((c->Helper.Type == MODEL_HELPER + 2 || o->movementType == Movement::Uniria) && !c->SafeZone)
		{
			SetAction(o, PLAYER_RIDE_SKILL);
			//SetAction(o, PLAYER_ATTACK_RIDE_ATTACK_MAGIC);
		}
		else if ((c->Helper.Type == MODEL_HELPER + 37 || o->movementType == Movement::Fenrir) && !c->SafeZone)
		{
...
bool CharacterAnimation(CHARACTER* c, OBJECT* o)
{
	...
	if (b)
	{
		...
		if (b->NumActions > 0)
		{
			...
			{
				PlaySpeed /= ReduceWithDelta(10.f);
			}
			...
			if (o->Type != MODEL_PLAYER)
			{
				...
				case MODEL_MONSTER01 + 64:
					if (o->CurrentAction == MONSTER01_DIE && o->AnimationFrame > 6)
						PlaySpeed *= ReduceWithDelta(4.0f);
					break;
				case MODEL_FACE:
				case MODEL_FACE + 1:
				case MODEL_FACE + 2:
				...
				case MODEL_FACE + 5:
				case MODEL_FACE + 6:
					PlaySpeed *= ReduceWithDelta(2.0f);
					break;
				}
			}
		...
		if (pModel)
		{
			...
			if (c->AttackTime <= 8)
			{
				...
				if (mtBone)
				{
					...
					o->m_vPosSword[0] += AdvanceByDelta(fDistance * sinf(o->Angle[2] * Q_PI / 180.0f));
					o->m_vPosSword[1] += AdvanceByDelta(-fDistance * cosf(o->Angle[2] * Q_PI / 180.0f));
				}
			}
...
void FallingCharacter(CHARACTER* c, OBJECT* o)
{
	...
	o->Direction[1] += AdvanceByDelta(o->Direction[0]);
	o->Gravity += AdvanceByDelta(o->Velocity);
	o->Velocity -= AdvanceByDelta(o->Direction[2]);
...
void PushingCharacter(CHARACTER* c, OBJECT* o)
{
	if (c->StormTime > 0)
	{
		o->Angle[2] += AdvanceByDelta(c->StormTime * 10);
		c->StormTime--;
	}
	if (c->JumpTime > 0)
	{
		...
		if (gMapManager->InChaosCastle() == true)
		{
			o->Position[0] += AdvanceByDelta((((float)c->TargetX) * TERRAIN_SCALE - o->Position[0]) * Speed);
			o->Position[1] += AdvanceByDelta((((float)c->TargetY) * TERRAIN_SCALE - o->Position[1]) * Speed);
			c->JumpTime++;
			if (c->JumpTime > 15)
			{
				SetPlayerStop(c);
		...
		{
			o->Position[0] += AdvanceByDelta((((float)c->TargetX + 0.5f) * TERRAIN_SCALE - o->Position[0]) * Speed);
			o->Position[1] += AdvanceByDelta((((float)c->TargetY + 0.5f) * TERRAIN_SCALE - o->Position[1]) * Speed);
...
void DeadCharacter(CHARACTER* c, OBJECT* o, BMD* b)
{
	...
	{
		c->Rot += AdvanceByDelta(0.05f);
	}
	else if (o->Type == MODEL_MONSTER01 + 64)
	{
		if (o->LifeTime >= 100)
			c->Rot += AdvanceByDelta(0.01f);
		else
			c->Rot += AdvanceByDelta(0.02f);
	}
	else
		c->Rot += AdvanceByDelta(0.02f);
	...
	if (c->Rot >= RotTime)
	{
		...
		if (o->Alpha >= 0.01f)
		{
			o->Position[2] -= AdvanceByDelta(0.4f);
		}
		else if (c != Hero)
		{
			o->Live = false;
...
void MoveCharacter(CHARACTER* c, OBJECT* o)
{
	...
	if (b)
	{
		...
		if (c->AttackTime >= 1.f)
		{
			AttackStage(c, o);
			AttackEffect(c);
			c->AttackTime += AdvanceByDelta(1.f);
		}
		...
		{
			c->m_iDeleteTime -= AdvanceByDelta(1.0f);
		}
...
void MoveCharacterPosition(CHARACTER* c)
{
	...
	{
		o->Position[2] += AdvanceByDelta(-absf(sinf(o->Timer)) * 70.f + 70.f);
	}
	o->Timer += AdvanceByDelta(0.15);
}
...
void RenderLinkObject(float x, float y, float z, CHARACTER* c, PART_t* f, int Type, int Level, int Option1, bool Link, bool Translate, int RenderType, bool bRightHandItem)
{
	...
	case MODEL_STAFF + 32:
	{
		...
		// Object->Timer 
		// Object->EyeRight 
		Object->Timer += AdvanceByDelta(0.01f);
		...
		{
			Object->EyeRight[0] *= ReduceWithDelta(0.95f);
			Object->EyeRight[1] *= ReduceWithDelta(0.95f);
			Object->EyeRight[2] *= ReduceWithDelta(0.95f);
		}
	...
	case MODEL_PLAYER:
		...
		if (gMapManager->InChaosCastle() == false)
		{
			if (c->ExtendState)
			{
				...
				//c->ExtendStateTime += AdvanceByDelta(1.f);
			}
	...
	for (int i = 0; i < MAX_CHARACTERS_CLIENT; ++i)
	{
		...
		/*if (o->Kind == 1 && SceneFlag == MAIN_SCENE && o->Live)
		{
			if (!g_isCharacterBuff(o, eBuff_Cloaking))
			{
				...
				EnableAlphaBlend();
				Vector(fLumi * vLight[0], fLumi * vLight[1], fLumi * vLight[2], vLight);
				RenderTerrainAlphaBitmap(BITMAP_GM_AURORA, o->Position[0], o->Position[1], size, size, vLight, WorldTime * 0.01f);
				RenderTerrainAlphaBitmap(BITMAP_GM_AURORA, o->Position[0], o->Position[1], 1.f, 1.f, vLight, -WorldTime * 0.01f);
				RenderAurora(BITMAP_GM_AURORA, RENDER_BRIGHT, o->Position[0], o->Position[1], 1.5f, 1.5f, vLight);
...
bool RenderCharacterBackItem(CHARACTER* c, OBJECT* o, bool bTranslate)
{
	...
	if (o->Type == MODEL_PLAYER)
	{
		...
		//--Phong FIX  Blood Castle GM Effect bug fix(5.2 sources)
		...
		if ((gMapManager->InBloodCastle() && c->EtcPart != 0) || (InBloodCastleCheckingGMAndWings(gMapManager->currentMap) && c->EtcPart != 0))
		{
			PART_t* w = &c->Wing;
