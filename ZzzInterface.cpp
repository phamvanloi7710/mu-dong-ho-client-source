...
//--Phong Fix Delay sau khi qua map để tránh lag khung hình
DWORD g_dwMapChangeTick = 0;
bool g_bMapDelayActive = false;
...
bool CheckAttack_Fenrir(CHARACTER* pCharacter)
{
	...
	if (::IsStrifeMap(gMapManager->currentMap) && pCharacter != Hero && pCharacter->m_byGensInfluence != Hero->m_byGensInfluence)
	{
		...
		{
			if (SEASON3B::IsRepeat(VK_CONTROL) == 1 || SEASON3B::IsCtrlPKActive() == true)
				return 1;				
		}
		if (strcmp(GuildMark[Hero->GuildMarkIndex].GuildName, GuildMark[pCharacter->GuildMarkIndex].GuildName))
		{
		...
		if (pCharacter->GuildRelationShip != GR_RIVAL && pCharacter->GuildRelationShip != GR_RIVALUNION)
		{
				...
				|| strcmp(GuildMark[Hero->GuildMarkIndex].GuildName, GuildMark[pCharacter->GuildMarkIndex].GuildName))
			{
				...
				else if (EnableGuildWar)
				{
					...
					((SEASON3B::IsRepeat(VK_CONTROL) != true && SEASON3B::IsCtrlPKActive() != true) || pCharacter == Hero))
				{
					success = false;
...
bool CheckAttack()
{
		...
		&& pCharacter->m_byGensInfluence != Hero->m_byGensInfluence)
	{
		...
		else if (SEASON3B::IsRepeat(VK_CONTROL) == true || SEASON3B::IsCtrlPKActive() == true)
		{
			if (EnableGuildWar)
				...
				else if (sc->GuildRelationShip != GR_RIVAL && sc->GuildRelationShip != GR_RIVALUNION)
				{
						...
						|| SEASON3B::IsRepeat(VK_MENU))
					{
						if ((signed int)sc->PK < PVP_MURDERER2 &&
							((SEASON3B::IsRepeat(VK_CONTROL) != true && SEASON3B::IsCtrlPKActive() != true) || sc == Hero))
						{
							if (gMapManager->IsCursedTemple())
							{
...
void MoveHero()
{
	...
	//--Phong Fix Delay sau khi qua map để tránh lag khung hình
	if (g_bMapDelayActive)
	{
		DWORD dwNow = GetTickCount();
		if (dwNow - g_dwMapChangeTick < 1000) // delay 1s
		{
			SetPlayerStop(Hero);
		...
		{
			g_bMapDelayActive = false; // hết delay, cho phép di chuyển
		}
	}
	...
	if (LoadingWorld > 0 && (--LoadingWorld == 9999998 || LoadingWorld == 29))
		SetPlayerStop(c);
	//--Phong Fix Delay sau khi qua map để tránh lag khung hình
	//--Phong: Kích hoạt delay sau khi qua map (chỉ khi vừa load xong)
	static bool s_bWasLoading = false;

	if (LoadingWorld > 0)
	{
		s_bWasLoading = true;
	}
	else if (s_bWasLoading && LoadingWorld == 0)
	{
		g_dwMapChangeTick = GetTickCount();
		g_bMapDelayActive = true;
		s_bWasLoading = false;
	}
...
void RenderHotKey()
{
	if (GetAsyncKeyState(VK_END) & 0x8000)
	{
		if (!EndKeyPressed)
		{
			AutoRightClick = !AutoRightClick;
			EndKeyPressed = true;

			if (AutoRightClick)
			{
				MouseRButton = true;   // ⭐ Thêm dòng này
			}
			else
			{
				MouseRButton = false;  // ⭐ Và tắt khi OFF
			}

			char Text[1200];
			if (AutoRightClick)
				sprintf(Text, "Chức năng Kẹp Chuột - Tự Động: BẬT");
			else
				sprintf(Text, "Chức năng Kẹp Chuột - Tự Động: TẮT");
...
void RenderInterface(bool Render)
{
	...
	// === Hiển thị trạng thái Auto Bơm Máu ===
	if (g_pCBInterface && g_pCBInterface->AutoHP)
	{
		g_pRenderText->SetBgColor(0);
		g_pRenderText->SetFont(g_hFontBold);
		g_pRenderText->SetTextColor(255, 153, 0, 255); // Cam đậm
		g_pRenderText->RenderText(5, 80, "Auto Bơm Máu:: ON", 0, 0, RT3_SORT_LEFT);
	}

	// === Hiển thị trạng thái Auto-Hold CTRL ===
	if (g_pNewKeyInput->IsCtrlAutoHeld())
	{
		g_pRenderText->SetBgColor(0);
		g_pRenderText->SetFont(g_hFontBold);
		g_pRenderText->SetTextColor(255, 215, 0, 255);
		g_pRenderText->RenderText(5, 90, "Auto-Hold CTRL: ON", 0, 0, RT3_SORT_LEFT);
	}

	// ✅ Thêm dòng này ở cuối hàm
	gPKSys.DrawNoticePKSystem();
		
	if (g_pCBInterface->Data[eWindow_DameBoss].OnShow)
	{
		//gDmgBoss.DAMEUPDATE();
		gDmgBoss.DAMEDRAW();
	}
...
//--Phong chỉnh sữa thanh máu monster
...
void RenderPartyHP()
{
	...
	for (size_t i = 0; i < MAX_CHARACTERS_CLIENT; i++)
	{
			...
			/*o->Kind == KIND_MONSTER*/) && pCharacter->Dead == 0 && pCharacter != Hero)
		{
			...
			// --- Bỏ qua NPC (lọc theo model Type) ---
			if ((o->Type == 1268 /*|| o->Type == 1273*/ ))    // khoảng ID của NPC thường
					  // Master, Elf Soldier
			{
				continue;
			}
			...
			if (i == SelectedCharacter || i == SelectedNpc) //--Phong Lệnh chỉ chuột vào player or monster mới hiện thanh máu
			{
				//--Phong Check type NPC
                //char info[64];
                //sprintf(info, "Type: %d | Kind: %d", o->Type, o->Kind);
                //g_pRenderText->RenderText(50, 50, info);
                //--Phong Check type NPC
...
#define GMT 7 // Múi giờ Việt Nam
	char ServerTime[30];
	sprintf_s(ServerTime, sizeof(ServerTime), "%02d:%02d:%02d",
		(timeinfo->tm_hour + GMT) % 24,
		timeinfo->tm_min,
		timeinfo->tm_sec);
	...
	{
		// Giả sử:
		//  - Hero->ID là chuỗi (const char*)
		//  - CharacterAttribute->Level là int
		//  - CharacterAttribute->btReset là số (BYTE/unsigned char) => hiển thị dưới dạng int
		//  - CharacterAttribute->ViewGHRS là "current RS" (giả sử numeric)
		//  - CharacterAttribute->SomeMaxRS là "max RS" (nếu không có, thay bằng 0 hoặc field phù hợp)
		//
		// Nếu tên trường maxRS khác trong struct của bạn, thay CharacterAttribute->MaxRS bằng trường đúng.

		int resets = (int)CharacterAttribute->btReset;
		int currentRS = (int)CharacterAttribute->ViewGHRS; // nếu ViewGHRS không phải số, đổi thành trường đúng
		int maxRS = 0; // <- thay bằng trường đúng nếu có, ví dụ CharacterAttribute->MaxRS

		sprintf_s(WindowName, sizeof(WindowName),
			"%s | NV: %s | LV: %d | Reset: %d | GHRS: %d | %s",
			CGMProtect::Instance()->GetWindowName(),
			Hero->ID,
	...
	// 🔹 Lấy handle thật của cửa sổ game
	HWND hWnd = gwinhandle ? gwinhandle->GethWnd() : FindWindowA(NULL, CGMProtect::Instance()->GetWindowName());
	if (hWnd)
	{
...
//	{
//		sprintf_s(WindowName, sizeof(WindowName),
//			"%s | NV: %s | LV: %d | %s | RS: %d/%d",
//			//"%s || NV: %s || LV: %d || RS: %d/%d || %s",
//			CGMProtect::Instance()->GetWindowName(),
//			Hero->ID,
//			CharacterAttribute->Level,
