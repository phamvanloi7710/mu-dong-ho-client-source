...
void CreateFrustrum(float Aspect, vec3_t position, bool wide = false);
...
inline int MapFileDecryptV16(uint8_t* pbyDst, uint8_t* pbySrc, int iSize)
{
	if (pbyDst)
	{
		uint8_t xorKey[16] = {
		0x8A, 0x4E, 0x22, 0x19, 0xC3, 0xB8, 0x5D, 0x6F,
		0x90, 0xDD, 0x77, 0xAB, 0x49, 0x11, 0x65, 0x3C
		};

		uint16_t wMapKey = 0xAC;

		for (int i = 0; i < iSize; ++i)
		{
			uint8_t key = xorKey[i % 16];

			uint8_t encryptedByte = pbySrc[i];
			uint8_t decryptedByte = (encryptedByte - wMapKey) ^ key;

			pbyDst[i] = decryptedByte;
			wMapKey = (encryptedByte + key + i) & 0xFF;
		}
	}
