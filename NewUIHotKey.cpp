	...
	if (CanUpdateKeyEventRelatedMyInventory() != true)
	{
		...
		else
		{
			...
			if (g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_INVENTORY) == false)
			{
				...
				if (PressKey('Y'))
				{
					static DWORD dwLastToggle = 0;  // Lưu thời điểm nhấn trước đó

					// Kiểm tra delay 300ms giữa 2 lần nhấn
					if (GetTickCount() - dwLastToggle > 300)
					{
						dwLastToggle = GetTickCount();

						if (g_pCBInterface)
						{
							g_pCBInterface->AutoHP = !g_pCBInterface->AutoHP;

							// === Gửi gói bật/tắt Auto HP ===
							PMSG_AUTO_HP_SEND pMsg;
							pMsg.header.set(0xD3, 0x35, sizeof(pMsg)); // Gửi đúng packet GS đang nhận
							pMsg.ThaoTac = g_pCBInterface->AutoHP ? 1 : 0;
							DataSend((LPBYTE)&pMsg, pMsg.header.size);

							// === Hiển thị thông báo ===
							char msg[64];
							sprintf(msg, "Auto HP: %s", g_pCBInterface->AutoHP ? "ON" : "OFF");
							g_pCBInterface->DrawMessage(0, msg);
						}
					}
				}
			...
			if (gmProtect->MenuSwitch == TRUE && SEASON3B::IsPress(VK_F4))
			{

				g_pNewUISystem->Toggle(INTERFACE_CUSTOM_MENU);
			...
			if (gmProtect->MenuSwitch == TRUE && SEASON3B::IsPress(VK_F9))
			{				 
