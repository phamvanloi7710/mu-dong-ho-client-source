...
#include "PL/PL_SystemConfigAndMemberInfo.h"
...
{
	#if USING_RECONNECT_ACCOUNT
		g_PLReconnect->ReconnectCreateConnection(IpAddr, Port);
	#endif // USING_RECONNECT_ACCOUNT
...
void ReceiveCharacterList(BYTE* ReceiveBuffer)
{
	...
	#if USING_RECONNECT_ACCOUNT
		g_PLReconnect->ReconnectOnCharacterList();
	#endif // USING_RECONNECT_ACCOUNT
...
{
	#if USING_RECONNECT_ACCOUNT
		g_PLReconnect->ReconnectOnCharacterInfo();
	#endif // USING_RECONNECT_ACCOUNT
	...
	CharacterAttribute->ViewGHRS = Data->ViewGHRS;
...
void ReceiveChangePlayer(BYTE* ReceiveBuffer)
{
	...
	if (pCharacter)
	{
		...
		BYTE ExtOption = Data->Item[4] & 0xFF; // Ensure we use the full byte value
		...
		case 0:
			if (Type == 0x1FFF)
			{
				...
				pCharacter->Weapon[0].ExtOption = 0;
			}
			else
			{
				pCharacter->Weapon[0].Type = MODEL_ITEM + Type;
				pCharacter->Weapon[0].Level = Level; // Store raw level (0-15) for proper rendering
				pCharacter->Weapon[0].Option1 = Option;
				pCharacter->Weapon[0].ExtOption = ExtOption;
			}
			break;
		case 1:
			if (Type == 0x1FFF)
			{
				...
				pCharacter->Weapon[1].ExtOption = 0;
				DeletePet(pCharacter);
			}
			else
			{
				pCharacter->Weapon[1].Type = MODEL_ITEM + Type;
				pCharacter->Weapon[1].Level = Level; // Store raw level (0-15) for proper rendering
				pCharacter->Weapon[1].Option1 = Option;
				pCharacter->Weapon[1].ExtOption = ExtOption;
				CreatePetDarkSpirit_Now(pCharacter);
		...
		case 2:
			...
			else
			{
				...
				pCharacter->BodyPart[BODYPART_HELM].Level = Level; // Store raw level (0-15) for proper rendering
		...
		case 3:
			...
			else
			{
				...
				pCharacter->BodyPart[BODYPART_ARMOR].Level = Level; // Store raw level (0-15) for proper rendering
		...
		case 4:
			...
			else
			{
				...
				pCharacter->BodyPart[BODYPART_PANTS].Level = Level; // Store raw level (0-15) for proper rendering
		...
		case 5:
			...
			else
			{
				...
				pCharacter->BodyPart[BODYPART_GLOVES].Level = Level; // Store raw level (0-15) for proper rendering
		...
		case 6:
			...
			else
			{
				...
				pCharacter->BodyPart[BODYPART_BOOTS].Level = Level; // Store raw level (0-15) for proper rendering
...
BOOL ReceiveNewCalculate(BYTE* ReceiveBuffer)
{
	...
	CharacterAttribute->btReset = Data->ViewReset;
	//--
...
void ReceiveChangeMapServerInfo(BYTE* ReceiveBuffer)
{
	...
	#if USING_RECONNECT_ACCOUNT
		g_PLReconnect->ReconnectOnMapServerMove(Data->m_vSvrInfo.m_szMapSvrIpAddress, Data->m_vSvrInfo.m_wMapSvrPort);
	#else
...
void ReceiveChangeMapServerResult(BYTE* ReceiveBuffer)
{
	LPPHEADER_DEFAULT Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
	#if USING_RECONNECT_ACCOUNT
		g_PLReconnect->ReconnectOnMapServerMoveAuth(Data->Value);
	#else
...
BOOL TranslateProtocol(int HeadCode, BYTE* ReceiveBuffer, int Size, BOOL bEncrypted)
{
	...
	case 0xF1:
	{
		...
		case 0x00: //receive join server
			...
			GCConnectClientRecv2((PMSG_CONNECT_CLIENT_RECV*)ReceiveBuffer);
			break;
		case 0x01: //receive log in
			#if USING_RECONNECT_ACCOUNT
				g_PLReconnect->ReconnectOnConnectAccount(Data->Value);
			#else
	...
	//g_ConsoleDebug->Write(MCD_RECEIVE, "Recv [0xF3][0x%02x]", subcode);
		...
		case 0xE1:
			...
			break;
		case 0xE2:
			GCNewHealthBarRecv((BYTE*)ReceiveBuffer);
			return 1;
			break;
		case 0xE3:
			//ReceiveNewCalculate(ReceiveBuffer);
...
#if USING_SYNCHRONIZE_DATA
			gPLSystemConfigAndMemberInfo.CGGetSystemConfigInfo();
#endif // USING_SYNCHRONIZE_DATA
			break;
		case 0xEF:
			if (g_pViewCharInfo)
			{
				g_pViewCharInfo->ReceiveViewCharacterInfo(ReceiveBuffer);
			}
			break;
		case 0xF3:
		...
		case 0xF5: gJwBanking.GCCustomBankInfoRecv(ReceiveBuffer);		break;
		}
		break;
	}
	case 0xF4:
	...
	case 0xD3:
	{
		switch (((ReceiveBuffer[0] == 0xC1) ? ReceiveBuffer[3] : ReceiveBuffer[4]))
		{
			...
			case 0x9A:
			{
				g_pNewUIMocNap->WsClientSend9A(ReceiveBuffer);
			}
			break;
			case 0x9B:
			{
				g_pNewUIMocNap->WsClientSend9B(ReceiveBuffer);

			}
			break;
			case 0x2E:
			{
				gPLNotification->OpenNotificationWindow(ReceiveBuffer);
...
#if USING_SYNCHRONIZE_DATA
	case 0x6C:
		gPLSystemConfigAndMemberInfo.GCSetSystemConfigInfo(ReceiveBuffer);
		break;
#endif // USING_SYNCHRONIZE_DATA
	...
	case 0x5A:
		g_RechargeMoney->QRCodeRecv(ReceiveBuffer);
		break;
	case 0x5B:
		//g_RechargeMoney->RecvPaymentInfoData((DGPaymentInfo*)lpMsg);
		break;
	case 0x5C:
	{
		int subcode = 0;
		...
		{
		case 0x00:
			g_RechargeMoney->RecvMyPaymentData(ReceiveBuffer);
			break;
		}
	}
...
#if USING_RESET_WITH_INTERFACE
	case 0x5F:

		break;
	case 0x6A:
		gResetSystem.LoadNormalReset((RESET_TABLE_INFO*)lpMsg);
		break;
	case 0x6B:
		gResetSystem.LoadVipReset((RESET_VIP_TABLE_INFO*)lpMsg);
		break;
#endif // USING_RESET_WITH_INTERFACE
	default:
		break;
	}
