...
#include <Windows.h>  // Thêm thư viện Windows.h
...
CmuConsoleDebug::CmuConsoleDebug() : m_bInit(false)
{
	...
	if (leaf::OpenConsoleWindow("Mu Debug Console Window"))
	{
		...
		// Thêm code để full màn hình console
		HWND hConsoleWnd = leaf::GetConsoleWndHandle();
		if (hConsoleWnd)
		{
			...
			// Đặt vị trí và kích thước console full màn hình
			SetWindowPos(hConsoleWnd, HWND_TOP, 0, 0, screenWidth, screenHeight, SWP_SHOWWINDOW);

			// Maximize cửa sổ console
			ShowWindow(hConsoleWnd, SW_MAXIMIZE);

			// Hoặc có thể dùng SW_SHOWMAXIMIZED để mở ngay ở chế độ maximized
			// ShowWindow(hConsoleWnd, SW_SHOWMAXIMIZED);
		}
	}
...
void CmuConsoleDebug::UpdateMainScene()
{
	...
	if (m_bInit)
	{
		if (SEASON3B::IsPress(VK_SHIFT) == TRUE)
		{
			if (PressKey(VK_F7))
			{
				bool bVisible = leaf::IsConsoleVisible();
				leaf::ShowConsole(!bVisible);

				// Nếu đang show console, thì maximize nó
				if (!bVisible)
				{
					HWND hConsoleWnd = leaf::GetConsoleWndHandle();
					if (hConsoleWnd)
...
bool CmuConsoleDebug::CheckCommand(const std::string& strCommand)
{
	...
	else if (strCommand.compare("$fullscreen") == NULL)  // Thêm lệnh fullscreen mới
	{
		HWND hConsoleWnd = leaf::GetConsoleWndHandle();
		if (hConsoleWnd)
		{
			...
			SetWindowPos(hConsoleWnd, HWND_TOP, 0, 0, screenWidth, screenHeight, SWP_SHOWWINDOW);
			ShowWindow(hConsoleWnd, SW_MAXIMIZE);
			Write(MCD_NORMAL, "Console set to fullscreen mode");
		}
		return true;
	}
	else if (strCommand.compare("$normal") == NULL)  // Thêm lệnh trở về kích thước bình thường
	{
		HWND hConsoleWnd = leaf::GetConsoleWndHandle();
		if (hConsoleWnd)
		{
			ShowWindow(hConsoleWnd, SW_RESTORE);
			SetWindowPos(hConsoleWnd, HWND_TOP, 100, 100, 800, 600, SWP_SHOWWINDOW);
			Write(MCD_NORMAL, "Console set to normal mode");
		}
		return true;
	}
