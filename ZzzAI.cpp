...
float MoveHumming(vec3_t Position, vec3_t Angle, vec3_t TargetPosition, float Turn, bool Fvg)
{
	...
	if (Fvg == true)
		Angle[2] = TurnAngle2(Angle[2], TargetAngle, AdvanceByDelta(Turn));
	else
	...
	if (Fvg == true)
		Angle[0] = TurnAngle2(Angle[0], TargetAngle, AdvanceByDelta(Turn));
	else
...
void MovePosition(vec3_t Position, vec3_t Angle, vec3_t Speed, bool Fvg)
{
	...
	if (Fvg == true)
	{
		Position[0] += AdvanceByDelta(Velocity[0]);
		Position[1] += AdvanceByDelta(Velocity[1]);
		Position[2] += AdvanceByDelta(Velocity[2]);
	}
	else
	{
