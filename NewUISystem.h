...
#include "Custom/PhongCustomDraw.h"	
...
namespace SEASON3B
{
	...
	public:
		...
		bool Update();
		bool Render();
		void RenderItem3DFree(float sx, float sy, float Width, float Height, int Type, int Level, int Option1, int ExtOption, bool PickUp, float Scale, bool FixY);	
	...
	private:
		...
		CNewUIMocNap* m_pNewUIMocNap;	
		CNewUIPhongDraw* m_pThangCustomDraw;	
		CNewUIViewCharInfo* m_NewUIViewCharInfo;	
        
		CNewUIPCDraw* m_pPCCustomDraw;
		CNewUIRankingTop* m_NewUIRankTop;
		
		#if USING_RECHARGE_MONEY
			PLCRechargeMoney* m_NewUIPurchaseMoney;
			PLCRechargeMoneyDetail* m_NewUIPurchaseMoneyDetail;
			PLCRechargeMoneyPaymentCancel* m_NewUIPurchaseMoneyCancelPayment;
		#endif // USING_RECHARGE_MONEY
		
		#if USING_RESET_WITH_GUI
			CPLResetSystem* m_NewUIResetSystem;
		#endif // USING_RESET_WITH_GUI

		PLCNotification* m_NewUINotification;
	...
	public:
		...
		CNewUIMocNap* Get_pNewUIMocNap()			const;	
		
		CNewUIPhongDraw* GetUI_NewThangCustomDraw() const;
		CNewUIViewCharInfo* GetUI_NewUIViewCharInfo() const;
																																											
		CNewUIPCDraw* GetUI_NewPCCustomDraw() const; 
		CNewUIRankingTop* GetUI_NewUIRankingTop() const;

		#if USING_RECHARGE_MONEY
			PLCRechargeMoney* GetUI_PLCRechargeMoney() const;
			PLCRechargeMoneyDetail* GetUI_PLCRechargeMoneyDetail() const;
			PLCRechargeMoneyPaymentCancel* GetUI_PLCRechargeMoneyPaymentCancel() const;
		#endif // USING_RECHARGE_MONEY

		#if USING_RESET_WITH_GUI
			CPLResetSystem* GetUI_CPLResetSystem() const;
		#endif // USING_RESET_WITH_GUI

		PLCNotification* GetUI_PLCNotification() const;
	};
}
...
#define g_pNewUIMenuOption			SEASON3B::CNewUISystem::GetInstance()

#define g_pNewUIMocNap				SEASON3B::CNewUISystem::GetInstance()->Get_pNewUIMocNap()	



#define g_pThangCustomDraw SEASON3B::CNewUISystem::GetInstance()->GetUI_NewThangCustomDraw()
#define g_pViewCharInfo		SEASON3B::CNewUISystem::GetInstance()->GetUI_NewUIViewCharInfo()


#define g_pPCCustomDraw SEASON3B::CNewUISystem::GetInstance()->GetUI_NewPCCustomDraw()
#define gNewUIRankingTop		SEASON3B::CNewUISystem::GetInstance()->GetUI_NewUIRankingTop()

#if USING_RECHARGE_MONEY
	#define g_RechargeMoney		SEASON3B::CNewUISystem::GetInstance()->GetUI_PLCRechargeMoney()
	#define g_RechargeMoneyDetail		SEASON3B::CNewUISystem::GetInstance()->GetUI_PLCRechargeMoneyDetail()
	#define g_RechargeMoneyCancelPayment		SEASON3B::CNewUISystem::GetInstance()->GetUI_PLCRechargeMoneyPaymentCancel()
#endif // USING_RECHARGE_MONEY

#if USING_RESET_WITH_GUI
	#define g_ResetSystem		SEASON3B::CNewUISystem::GetInstance()->GetUI_CPLResetSystem()
#endif // USING_RESET_WITH_GUI

#define gPLNotification		SEASON3B::CNewUISystem::GetInstance()->GetUI_PLCNotification()